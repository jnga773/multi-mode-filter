...
!==============================================================================!
SUBROUTINE SpectrumPeakTransitions(Omega_in, alpha_in, delta_in, xi_in, &
                                 & frequencies_out)
  ...
  ! Calculate the eigenvalues of the Hamiltonian in matrix form, and then
  ! calculate the peak transition frequencies for the power spectrum.
  ...
  REAL(KIND=8), DIMENSION(4), INTENT(OUT)    :: frequencies_out
  ! REAL(KIND=8), DIMENSION(3) , INTENT(OUT)  :: eigvals_out

  !---------------------!
  !     OTHER STUFF     !
  !---------------------!
  INTEGER                                    :: j, x
  ! Matrix for the Hamiltonian
  REAL(KIND=8), DIMENSION(3, 3)              :: H_A
  ! Eigenvalues of matrix M
  REAL(KIND=8), DIMENSION(3)                 :: eigvals_real, eigvals_imag
  ! The dressed state frequencies
  REAL(KIND=8)                               :: w_l, w_m, w_u
  ...
  INTEGER                                    :: LWORK
  ! Work space array
  REAL(KIND=8), DIMENSION(LWMAX)             :: WORK
  REAL(KIND=8), DIMENSION(2*3)               :: RWORK
  ! S, and S^{-1} matrix for diagonalising eigenvectors
  REAL(KIND=8), DIMENSION(3, 3)              :: S, Sinv
  ! Info IO
  INTEGER                                    :: INFO
  ...
  !============================================================================!
  ! Set the Hamiltonian in matrix form
  H_A(1, 1) = 0.0d0
  H_A(1, 2) = 0.5d0 * Omega_in
  H_A(1, 3) = 0.0d0

  H_A(2, 1) = 0.5d0 * Omega_in
  H_A(2, 2) = -((0.5d0 * alpha_in) +  delta_in)
  H_A(2, 3) = 0.5d0 * xi_in * Omega_in

  H_A(3, 1) = 0.0d0
  H_A(3, 2) = 0.5d0 * xi_in * Omega_in
  H_A(3, 3) = -2.0d0 * delta_in

  ! Calculate eigenvalues and eigenvectors (Optimal LWORK = 264)
  LWORK = 264
  CALL dGEEV('N', 'V', 3, H_A, 3, eigvals_real, eigvals_imag, S, 3, S, 3, WORK, LWORK, INFO)
  ! call zGEEV('N', 'V', 3, H_A, 3, eigvals_out, S, 3, S, 3, WORK, LWORK, RWORK, INFO)
  ! Check convergence
  IF (INFO .GT. 0) THEN
     PRINT*, "dGEEV failed ON eigenvalues/vectors of Mat_OG"
     STOP
  END IF

  ! eigvals_out = eigvals_real

  ! Save the frequencies
  w_u = eigvals_real(1); w_m = eigvals_real(2); w_l = eigvals_real(3)

  ! ! Print to console
  ! PRINT*, "Hamiltonian Dressed State Frequencies:"
  ! WRITE(*, '(A6, F15.5)') 'w_u = ', w_u
  ! WRITE(*, '(A6, F15.5)') 'w_m = ', w_m
  ! WRITE(*, '(A6, F15.5)') 'w_l = ', w_l
  ! PRINT*, ""

  ! Calculate and save the dressed state transition frequencies
  frequencies_out(1) = 0.0d0
  frequencies_out(2) = ABS(w_m - w_l)
  frequencies_out(3) = ABS(w_m - w_u)
  frequencies_out(4) = ABS(w_u - w_l)

  ! Print to console
  PRINT*, "Dressed State Transition Frequecies"
  WRITE(*, '(A21, F30.27)') ' w_0 = |w_d - w_d| = ', frequencies_out(1)
  WRITE(*, '(A21, F30.27)') 'w_+1 = |w_m - w_l| = ', frequencies_out(2)
  WRITE(*, '(A21, F30.27)') 'w_+2 = |w_m - w_u| = ', frequencies_out(3)
  WRITE(*, '(A21, F30.27)') 'w_+3 = |w_u - w_l| = ', frequencies_out(4)

END SUBROUTINE SpectrumPeakTransitions
...
! Subroutine to calculate the steady states for the atom-filter operator moments
SUBROUTINE SteadyStateMoments_G1(Gamma_in, Omega_in, alpha_in, delta_in, xi_in, &
                               & epsilon_in, N_in, phase_in, &
                               & w0_in, kappa_in, dw_in, &
                               & photon_out, sigma_out, &
                               & f1_out, f1sig_out, &
...
SUBROUTINE G1_InitialConditions(Gamma_in, Omega_in, alpha_in, delta_in, xi_in, &
  ...
  !                       CALCULATE STEADY-STATE MOMENTS                       !
  !============================================================================!
  CALL SteadyStateMoments_G1(Gamma_in, Omega_in, alpha_in, delta_in, xi_in, &
                           & epsilon_in, N_in, phase_in, &
                           & w0_in, kappa_in, dw_in, &
...
SUBROUTINE G2_InitialValue(Gamma_in, Omega_in, alpha_in, delta_in, xi_in, &
                         & epsilon_in, N_in, phase_in, &
                         & w0_in, kappa_in, dw_in, &
                         & g2_initial_out)

...
DO j = -N, N
  ...
  ! < a_{j} > !
  !-----------!
  cav1_ss(j, a) = -gkl(j, a) * sigma_ss(ge) + &
  ...
  ! < a^{\dagger}_{j} > !
  !---------------------!
  cav1_ss(j, at) = -CONJG(gkl(j, a)) * sigma_ss(eg) + &
...
DO k = -N, N
  DO j = -N, N
    ...
    cav2_ss(j, k, ab) = -gkl(j, a) * cavsig2b_ss(k, b, ge) + &
                      ...
                      & -gkl(k, b) * cavsig2_ss(j, a, ge) + &
                      & -gkl(k, b) * xi * cavsig2_ss(j, a, ef)
    ...
    !-------------------------------------!
    cav2_ss(j, k, atbt) = -CONJG(gkl(j, a)) * cavsig2b_ss(k, bt, eg) + &
                        & -CONJG(gkl(j, a)) * xi * cavsig2b_ss(k, bt, fe) + &
                        & -CONJG(gkl(k, b)) * cavsig2_ss(j, at, eg) + &
                        & -CONJG(gkl(k, b)) * xi * cavsig2_ss(j, at, fe)
    ...
    ! < a^{\dagger}_{j} a_{k} > !
    !---------------------------!
    cav2_ss(j, k, ata) = -CONJG(gkl(j, a)) * cavsig2_ss(k, a, eg) + &
                       & -CONJG(gkl(j, a)) * xi * cavsig2_ss(k, a, fe) + &
                       & -gkl(k, a) * cavsig2_ss(j, at, ge) + &
                       & -gkl(k, a) * xi * cavsig2_ss(j, at, ef)
    ...
    cav2_ss(j, k, atb) = -CONJG(gkl(j, a)) * cavsig2b_ss(k, b, eg) + &
                       ...
                       & -gkl(k, b) * cavsig2_ss(j, at, ge) + &
                       & -gkl(k, b) * xi * cavsig2_ss(j, at, ef)
    ...
    ! < b^{\dagger}_{j} a_{k} > !
    !---------------------------!
    cav2_ss(j, k, bta) = -CONJG(gkl(j, b)) * cavsig2_ss(k, a, eg) + &
                       & -CONJG(gkl(j, b)) * xi * cavsig2_ss(k, a, fe) + &
    ...
    B_vec(2) = -gkl(j, a) * xi * cavsig2b_ss(k, b, gf) + &
             & -gkl(k, b) * xi * cavsig2_ss(j, a, gf)
    B_vec(3) = -gkl(j, a) * cavsig2b_ss(k, b, ee) + &
             & -gkl(k, b) * cavsig2_ss(j, a, ee)
    ...
    B_vec(6) = -i * xi * 0.5d0 * Omega * cav2_ss(j, k, ab) + &
             ...
             & -gkl(j, a) * xi * cav1_ss(k, b) + &
             & gkl(k, b) * xi * cavsig2_ss(j, a, gg) + &
             & gkl(k, b) * xi * cavsig2_ss(j, a, ee) + &
             & -gkl(k, b) * xi * cav1_ss(j, a)
    B_vec(7) = 0.0d0
    B_vec(8) = -gkl(j, a) * cavsig2b_ss(k, b, fe) + &
             & -gkl(k, b) * cavsig2_ss(j, a, fe)
    ...
    B_vec(2) = -CONJG(gkl(j, a)) * cavsig2b_ss(k, bt, ee) + &
             & -CONJG(gkl(k, b)) * cavsig2_ss(j, at, ee)
    B_vec(3) = -CONJG(gkl(j, a)) * xi * cavsig2b_ss(k, bt, fg) + &
             & -CONJG(gkl(k, b)) * xi * cavsig2_ss(j, at, fg)
    ...
    B_vec(5) = i * xi * 0.5d0 * Omega * cav2_ss(j, k, atbt) + &
             ...
             & CONJG(gkl(k, b)) * xi * cavsig2_ss(j, at, gg) + &
             & CONJG(gkl(k, b)) * xi * cavsig2_ss(j, at, ee) + &
             & -CONJG(gkl(k, b)) * xi * cav1_ss(j, at)
    B_vec(6) = -i * xi * 0.5d0 * Omega * cav2_ss(j, k, atbt)
    B_vec(7) = -CONJG(gkl(j, a)) * cavsig2b_ss(k, bt, ef) + &
             & -CONJG(gkl(k, b)) * cavsig2_ss(j, at, ef)
    B_vec(8) = 0.0d0
    ...
    ! Set the non-homogeneous vector
    B_vec = 0.0d0
    B_vec(1) = -CONJG(gkl(j, a)) * cavsig2_ss(k, a, eg) + &
             & -gkl(k, a) * cavsig2_ss(j, at, ge)
    B_vec(2) = -CONJG(gkl(j, a)) * cavsig2_ss(k, a, ee) + &
             & -gkl(k, a) * xi * cavsig2_ss(j, at, gf)
    B_vec(3) = -CONJG(gkl(j, a)) * xi * cavsig2_ss(k, a, fg) + &
             & -gkl(k, a) * cavsig2_ss(j, at, ee)
    B_vec(4) = Gamma * (xi ** 2) * cav2_ss(j, k, ata) + &
             & -CONJG(gkl(j, a)) * xi * cavsig2_ss(k, a, fe) + &
             & -gkl(k, a) * xi * cavsig2_ss(j, at, ef)
    B_vec(5) = i * xi * 0.5d0 * Omega * cav2_ss(j, k, ata) + &
             & CONJG(gkl(j, a)) * xi * cavsig2_ss(k, a, gg) + &
             & CONJG(gkl(j, a)) * xi * cavsig2_ss(k, a, ee) + &
             & -CONJG(gkl(j, a)) * xi * cav1_ss(k, a)
    B_vec(6) = -i * xi * 0.5d0 * Omega * cav2_ss(j, k, ata) + &
             & gkl(k, a) * xi * cavsig2_ss(j, at, gg) + &
             & gkl(k, a) * xi * cavsig2_ss(j, at, ee) + &
             & -gkl(k, a) * xi * cav1_ss(j, at)
    B_vec(7) = -CONJG(gkl(j, a)) * cavsig2_ss(k, a, ef)
    B_vec(8) = -gkl(k, a) * cavsig2_ss(j, at, fe)
    ...
    B_vec(2) = -CONJG(gkl(j, a)) * cavsig2b_ss(k, b, ee) + &
             & -gkl(k, b) * xi * cavsig2_ss(j, at, gf)
    B_vec(3) = -CONJG(gkl(j, a)) * xi * cavsig2b_ss(k, b, fg) + &
             & -gkl(k, b) * cavsig2_ss(j, at, ee)
    ...
    B_vec(6) = -i * xi * 0.5d0 * Omega * cav2_ss(j, k, atb) + &
             & gkl(k, b) * xi * cavsig2_ss(j, at, gg) + &
             & gkl(k, b) * xi * cavsig2_ss(j, at, ee) + &
             & -gkl(k, b) * xi * cav1_ss(j, at)
    B_vec(7) = -CONJG(gkl(j, a)) * cavsig2b_ss(k, b, ef)
    B_vec(8) = -gkl(k, b) * cavsig2_ss(j, at, fe)
    ...
    ! Set the non-homogeneous vector
    B_vec = 0.0d0
    B_vec(1) = -CONJG(gkl(j, b)) * cavsig2_ss(k, a, eg) + &
             & -gkl(k, a) * cavsig2b_ss(j, bt, ge)
    B_vec(2) = -CONJG(gkl(j, b)) * cavsig2_ss(k, a, ee) + &
             & -gkl(k, a) * xi * cavsig2b_ss(j, bt, gf)
    B_vec(3) = -CONJG(gkl(j, b)) * xi * cavsig2_ss(k, a, fg) + &
             & -gkl(k, a) * cavsig2b_ss(j, bt, ee)
    B_vec(4) = Gamma * (xi ** 2) * cav2_ss(j, k, bta) + &
             & -CONJG(gkl(j, b)) * xi * cavsig2_ss(k, a, fe) + &
             & -gkl(k, a) * xi * cavsig2b_ss(j, bt, ef)
    B_vec(5) = i * xi * 0.5d0 * Omega * cav2_ss(j, k, bta) + &
             & CONJG(gkl(j, b)) * xi * cavsig2_ss(k, a, gg) + &
             & CONJG(gkl(j, b)) * xi * cavsig2_ss(k, a, ee) + &
             & -CONJG(gkl(j, b)) * xi * cav1_ss(k, a)
    ...
    B_vec(7) = -CONJG(gkl(j, b)) * cavsig2_ss(k, a, ef)
...
DO t = 0, tau_steps
  ...
  ! IF (photon_ss(a) .NE. 0.0d0) THEN
  !   moment_out = moment_out / (photon_ss(a) ** 2)
  ! END IF

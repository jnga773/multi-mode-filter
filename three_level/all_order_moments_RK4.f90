...
DO t = 0, t_steps
  ...
  DO j = -N, N
    ...
    B = 0.0d0
    B(1) = -gkl(j) * sigma(ge)
    B(2) = -gkl(j) * xi * sigma(gf)
    B(3) = -gkl(j) * sigma(ee)
    B(4) = Gamma * (xi ** 2) * cav1(j, a) + &
         & -gkl(j) * xi * sigma(ef)
    B(5) = i * xi * 0.5d0 * Omega * cav1(j, a)
    B(6) = -i * xi * 0.5d0 * Omega * cav1(j, a) + &
         & gkl(j) * xi * sigma(gg) + &
         & gkl(j) * xi * sigma(ee) + &
         & -gkl(j) * xi
    B(7) = 0.0d0
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -gkl(j) * (sigma(ge) + 0.5d0 * k1_sigma(ge))
    B(2) = -gkl(j) * xi * (sigma(gf) + 0.5d0 * k1_sigma(gf))
    B(3) = -gkl(j) * (sigma(ee) + 0.5d0 * k1_sigma(ee))
    B(4) = Gamma * (xi ** 2) * (cav1(j, a) + 0.5d0 * k1_cav1(j, a)) + &
         & -gkl(j) * xi * (sigma(ef) + 0.5d0 * k1_sigma(ef))
    B(5) = i * xi * 0.5d0 * Omega * (cav1(j, a) + 0.5d0 * k1_cav1(j, a))
    B(6) = -i * xi * 0.5d0 * Omega * (cav1(j, a) + 0.5d0 * k1_cav1(j, a)) + &
         & gkl(j) * xi * (sigma(gg) + 0.5d0 * k1_sigma(gg)) + &
         & gkl(j) * xi * (sigma(ee) + 0.5d0 * k1_sigma(ee)) + &
         & -gkl(j) * xi
    B(7) = 0.0d0
    B(8) = -gkl(j) * (sigma(fe) + 0.5d0 * k1_sigma(fe))
    ! Calculate k2
    k2_cavsig2(j, a, :) = dt * (MATMUL(Mat, (cavsig2(j, a, :) + 0.5d0 * k1_cavsig2(j, a, :))) + B)

    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -gkl(j) * (sigma(ge) + 0.5d0 * k2_sigma(ge))
    B(2) = -gkl(j) * xi * (sigma(gf) + 0.5d0 * k2_sigma(gf))
    B(3) = -gkl(j) * (sigma(ee) + 0.5d0 * k2_sigma(ee))
    B(4) = Gamma * (xi ** 2) * (cav1(j, a) + 0.5d0 * k2_cav1(j, a)) + &
         & -gkl(j) * xi * (sigma(ef) + 0.5d0 * k2_sigma(ef))
    B(5) = i * xi * 0.5d0 * Omega * (cav1(j, a) + 0.5d0 * k2_cav1(j, a))
    B(6) = -i * xi * 0.5d0 * Omega * (cav1(j, a) + 0.5d0 * k2_cav1(j, a)) + &
         & gkl(j) * xi * (sigma(gg) + 0.5d0 * k2_sigma(gg)) + &
         & gkl(j) * xi * (sigma(ee) + 0.5d0 * k2_sigma(ee)) + &
         & -gkl(j) * xi
    B(7) = 0.0d0
    B(8) = -gkl(j) * (sigma(fe) + 0.5d0 * k2_sigma(fe))
    ! Calculate k3
    k3_cavsig2(j, a, :) = dt * (MATMUL(Mat, (cavsig2(j, a, :) + 0.5d0 * k2_cavsig2(j, a, :))) + B)

    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -gkl(j) * (sigma(ge) + k3_sigma(ge))
    B(2) = -gkl(j) * xi * (sigma(gf) + k3_sigma(gf))
    B(3) = -gkl(j) * (sigma(ee) + k3_sigma(ee))
    B(4) = Gamma * (xi ** 2) * (cav1(j, a) + k3_cav1(j, a)) + &
         & -gkl(j) * xi * (sigma(ef) + k3_sigma(ef))
    B(5) = i * xi * 0.5d0 * Omega * (cav1(j, a) + k3_cav1(j, a))
    B(6) = -i * xi * 0.5d0 * Omega * (cav1(j, a) + k3_cav1(j, a)) + &
         & gkl(j) * xi * (sigma(gg) + k3_sigma(gg)) + &
         & gkl(j) * xi * (sigma(ee) + k3_sigma(ee)) + &
         & -gkl(j) * xi
    B(7) = 0.0d0
    B(8) = -gkl(j) * (sigma(fe) + k3_sigma(fe))
    ! Calculate k4
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -CONJG(gkl(j)) * sigma(eg)
    B(2) = -CONJG(gkl(j)) * sigma(ee)
    B(3) = -CONJG(gkl(j)) * xi * sigma(fg)
    B(4) = Gamma * (xi ** 2) * cav1(j, at) + &
         & -CONJG(gkl(j)) * xi * sigma(fe)
    B(5) = i * xi * 0.5d0 * Omega * cav1(j, at) + &
         & CONJG(gkl(j)) * xi * sigma(gg) + &
         & CONJG(gkl(j)) * xi * sigma(ee) + &
         & -CONJG(gkl(j)) * xi
    B(6) = -i * xi * 0.5d0 * Omega * cav1(j, at)
    B(7) = -CONJG(gkl(j)) * sigma(ef)
    B(8) = 0.0d0
    ! Calculate k1
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -CONJG(gkl(j)) * (sigma(eg) + 0.5d0 * k1_sigma(eg))
    B(2) = -CONJG(gkl(j)) * (sigma(ee) + 0.5d0 * k1_sigma(ee))
    B(3) = -CONJG(gkl(j)) * xi * (sigma(fg) + 0.5d0 * k1_sigma(fg))
    B(4) = Gamma * (xi ** 2) * (cav1(j, at) + 0.5d0 * k1_cav1(j, at)) + &
         & -CONJG(gkl(j)) * xi * (sigma(fe) + 0.5d0 * k1_sigma(fe))
    B(5) = i * xi * 0.5d0 * Omega * (cav1(j, at) + 0.5d0 * k1_cav1(j, at)) + &
         & CONJG(gkl(j)) * xi * (sigma(gg) + 0.5d0 * k1_sigma(gg)) + &
         & CONJG(gkl(j)) * xi * (sigma(ee) + 0.5d0 * k1_sigma(ee)) + &
         & -CONJG(gkl(j)) * xi
    B(6) = -i * xi * 0.5d0 * Omega * (cav1(j, at) + 0.5d0 * k1_cav1(j, at))
    B(7) = -CONJG(gkl(j)) * (sigma(ef) + 0.5d0 * k1_sigma(ef))
    B(8) = 0.0d0
    ! Calculate k2
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -CONJG(gkl(j)) * (sigma(eg) + 0.5d0 * k2_sigma(eg))
    B(2) = -CONJG(gkl(j)) * (sigma(ee) + 0.5d0 * k2_sigma(ee))
    B(3) = -CONJG(gkl(j)) * xi * (sigma(fg) + 0.5d0 * k2_sigma(fg))
    B(4) = Gamma * (xi ** 2) * (cav1(j, at) + 0.5d0 * k2_cav1(j, at)) + &
         & -CONJG(gkl(j)) * xi * (sigma(fe) + 0.5d0 * k2_sigma(fe))
    B(5) = i * xi * 0.5d0 * Omega * (cav1(j, at) + 0.5d0 * k2_cav1(j, at)) + &
         & CONJG(gkl(j)) * xi * (sigma(gg) + 0.5d0 * k2_sigma(gg)) + &
         & CONJG(gkl(j)) * xi * (sigma(ee) + 0.5d0 * k2_sigma(ee)) + &
         & -CONJG(gkl(j)) * xi
    B(6) = -i * xi * 0.5d0 * Omega * (cav1(j, at) + 0.5d0 * k2_cav1(j, at))
    B(7) = -CONJG(gkl(j)) * (sigma(ef) + 0.5d0 * k2_sigma(ef))
    B(8) = 0.0d0
    ! Calculate k3
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -CONJG(gkl(j)) * (sigma(eg) + k3_sigma(eg))
    B(2) = -CONJG(gkl(j)) * (sigma(ee) + k3_sigma(ee))
    B(3) = -CONJG(gkl(j)) * xi * (sigma(fg) + k3_sigma(fg))
    B(4) = Gamma * (xi ** 2) * (cav1(j, at) + k3_cav1(j, at)) + &
         & -CONJG(gkl(j)) * xi * (sigma(fe) + k3_sigma(fe))
    B(5) = i * xi * 0.5d0 * Omega * (cav1(j, at) + k3_cav1(j, at)) + &
         & CONJG(gkl(j)) * xi * (sigma(gg) + k3_sigma(gg)) + &
         & CONJG(gkl(j)) * xi * (sigma(ee) + k3_sigma(ee)) + &
         & -CONJG(gkl(j)) * xi
    B(6) = -i * xi * 0.5d0 * Omega * (cav1(j, at) + k3_cav1(j, at))
    B(7) = -CONJG(gkl(j)) * (sigma(ef) + k3_sigma(ef))
    B(8) = 0.0d0
    ! Calculate k4
  ...
  DO j = -N, N
    DO k = -N, N
      ...
      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -gkl(j) * cavsig2(k, a, ge) + &
           & -gkl(k) * cavsig2(j, a, ge)
      B(2) = -gkl(j) * xi * cavsig2(k, a, gf) + &
           & -gkl(k) * xi * cavsig2(j, a, gf)
      B(3) = -gkl(j) * cavsig2(k, a, ee) + &
           & -gkl(k) * cavsig2(j, a, ee)
      B(4) = Gamma * (xi ** 2) * cav2(j, k, a) + &
           & -gkl(j) * xi * cavsig2(k, a, ef) + &
           & -gkl(k) * xi * cavsig2(j, a, ef)
      B(5) = i * xi * 0.5d0 * Omega * cav2(j, k, a)
      B(6) = -i * xi * 0.5d0 * Omega * cav2(j, k, a) + &
           ...
           & -gkl(k) * xi * cav1(j, a)
      B(7) = 0.0d0
      B(8) = -gkl(j) * cavsig2(k, a, fe) + &
           & -gkl(k) * cavsig2(j, a, fe)
      ! Calculate k1
      k1_cavsig3(j, k, a, :) = dt * (MATMUL(Mat, cavsig3(j, k, a, :)) + B)

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -gkl(j) * (cavsig2(k, a, ge) + 0.5d0 * k1_cavsig2(k, a, ge)) + &
           & -gkl(k) * (cavsig2(j, a, ge) + 0.5d0 * k1_cavsig2(j, a, ge))
      B(2) = -gkl(j) * xi * (cavsig2(k, a, gf) + 0.5d0 * k1_cavsig2(k, a, gf)) + &
           & -gkl(k) * xi * (cavsig2(j, a, gf) + 0.5d0 * k1_cavsig2(j, a, gf))
      B(3) = -gkl(j) * (cavsig2(k, a, ee) + 0.5d0 * k1_cavsig2(k, a, ee)) + &
           & -gkl(k) * (cavsig2(j, a, ee) + 0.5d0 * k1_cavsig2(j, a, ee))
      B(4) = Gamma * (xi ** 2) * (cav2(j, k, a) + 0.5d0 * k1_cav2(j, k, a)) + &
           & -gkl(j) * xi * (cavsig2(k, a, ef) + 0.5d0 * k1_cavsig2(k, a, ef)) + &
           & -gkl(k) * xi * (cavsig2(j, a, ef) + 0.5d0 * k1_cavsig2(j, a, ef))
      B(5) = i * xi * 0.5d0 * Omega * (cav2(j, k, a) + 0.5d0 * k1_cav2(j, k, a))
      B(6) = -i * xi * 0.5d0 * Omega * (cav2(j, k, a) + 0.5d0 * k1_cav2(j, k, a)) + &
      ...
      B(7) = 0.0d0
      B(8) = -gkl(j) * (cavsig2(k, a, fe) + 0.5d0 * k1_cavsig2(k, a, fe)) + &
           & -gkl(k) * (cavsig2(j, a, fe) + 0.5d0 * k1_cavsig2(j, a, fe))
      ! Calculate k2
      k2_cavsig3(j, k, a, :) = dt * (MATMUL(Mat, (cavsig3(j, k, a, :) + 0.5d0 * k1_cavsig3(j, k, a, :))) + B)

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -gkl(j) * (cavsig2(k, a, ge) + 0.5d0 * k2_cavsig2(k, a, ge)) + &
           & -gkl(k) * (cavsig2(j, a, ge) + 0.5d0 * k2_cavsig2(j, a, ge))
      B(2) = -gkl(j) * xi * (cavsig2(k, a, gf) + 0.5d0 * k2_cavsig2(k, a, gf)) + &
           & -gkl(k) * xi * (cavsig2(j, a, gf) + 0.5d0 * k2_cavsig2(j, a, gf))
      B(3) = -gkl(j) * (cavsig2(k, a, ee) + 0.5d0 * k2_cavsig2(k, a, ee)) + &
           & -gkl(k) * (cavsig2(j, a, ee) + 0.5d0 * k2_cavsig2(j, a, ee))
      B(4) = Gamma * (xi ** 2) * (cav2(j, k, a) + 0.5d0 * k2_cav2(j, k, a)) + &
           & -gkl(j) * xi * (cavsig2(k, a, ef) + 0.5d0 * k2_cavsig2(k, a, ef)) + &
           & -gkl(k) * xi * (cavsig2(j, a, ef) + 0.5d0 * k2_cavsig2(j, a, ef))
      B(5) = i * xi * 0.5d0 * Omega * (cav2(j, k, a) + 0.5d0 * k2_cav2(j, k, a))
      B(6) = -i * xi * 0.5d0 * Omega * (cav2(j, k, a) + 0.5d0 * k2_cav2(j, k, a)) + &
      ...
      B(7) = 0.0d0
      B(8) = -gkl(j) * (cavsig2(k, a, fe) + 0.5d0 * k2_cavsig2(k, a, fe)) + &
           & -gkl(k) * (cavsig2(j, a, fe) + 0.5d0 * k2_cavsig2(j, a, fe))
      ! Calculate k3
      k3_cavsig3(j, k, a, :) = dt * (MATMUL(Mat, (cavsig3(j, k, a, :) + 0.5d0 * k2_cavsig3(j, k, a, :))) + B)

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -gkl(j) * (cavsig2(k, a, ge) + k3_cavsig2(k, a, ge)) + &
           & -gkl(k) * (cavsig2(j, a, ge) + k3_cavsig2(j, a, ge))
      B(2) = -gkl(j) * xi * (cavsig2(k, a, gf) + k3_cavsig2(k, a, gf)) + &
           & -gkl(k) * xi * (cavsig2(j, a, gf) + k3_cavsig2(j, a, gf))
      B(3) = -gkl(j) * (cavsig2(k, a, ee) + k3_cavsig2(k, a, ee)) + &
           & -gkl(k) * (cavsig2(j, a, ee) + k3_cavsig2(j, a, ee))
      B(4) = Gamma * (xi ** 2) * (cav2(j, k, a) + k3_cav2(j, k, a)) + &
           & -gkl(j) * xi * (cavsig2(k, a, ef) + k3_cavsig2(k, a, ef)) + &
           & -gkl(k) * xi * (cavsig2(j, a, ef) + k3_cavsig2(j, a, ef))
      B(5) = i * xi * 0.5d0 * Omega * (cav2(j, k, a) + k3_cav2(j, k, a))
      B(6) = -i * xi * 0.5d0 * Omega * (cav2(j, k, a) + k3_cav2(j, k, a)) + &
      ...
      B(7) = 0.0d0
      B(8) = -gkl(j) * (cavsig2(k, a, fe) + k3_cavsig2(k, a, fe)) + &
           & -gkl(k) * (cavsig2(j, a, fe) + k3_cavsig2(j, a, fe))
      ! Calculate k4
      k4_cavsig3(j, k, a, :) = dt * (MATMUL(Mat, (cavsig3(j, k, a, :) + k3_cavsig3(j, k, a, :))) + B)
      ...
      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * cavsig2(k, at, eg) + &
           & -CONJG(gkl(k)) * cavsig2(j, at, eg)
      B(2) = -CONJG(gkl(j)) * cavsig2(k, at, ee) + &
           & -CONJG(gkl(k)) * cavsig2(j, at, ee)
      B(3) = -CONJG(gkl(j)) * xi * cavsig2(k, at, fg) + &
           & -CONJG(gkl(k)) * xi * cavsig2(j, at, fg)
      B(4) = Gamma * (xi ** 2) * cav2(j, k, at) + &
           & -CONJG(gkl(j)) * xi * cavsig2(k, at, fe) + &
           & -CONJG(gkl(k)) * xi * cavsig2(j, at, fe)
      B(5) = i * xi * 0.5d0 * Omega * cav2(j, k, at) + &
      ...
      B(6) = -i * xi * 0.5d0 * Omega * cav2(j, k, at)
      B(7) = -CONJG(gkl(j)) * cavsig2(k, at, ef) + &
           & -CONJG(gkl(k)) * cavsig2(j, at, ef)
      B(8) = 0.0d0
      ! Calculate k1
      k1_cavsig3(j, k, at, :) = dt * (MATMUL(Mat, cavsig3(j, k, at, :)) + B)

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * (cavsig2(k, at, eg) + 0.5d0 * k1_cavsig2(k, at, eg)) + &
           & -CONJG(gkl(k)) * (cavsig2(j, at, eg) + 0.5d0 * k1_cavsig2(j, at, eg))
      B(2) = -CONJG(gkl(j)) * (cavsig2(k, at, ee) + 0.5d0 * k1_cavsig2(k, at, ee)) + &
           & -CONJG(gkl(k)) * (cavsig2(j, at, ee) + 0.5d0 * k1_cavsig2(j, at, ee))
      B(3) = -CONJG(gkl(j)) * xi * (cavsig2(k, at, fg) + 0.5d0 * k1_cavsig2(k, at, fg)) + &
           & -CONJG(gkl(k)) * xi * (cavsig2(j, at, fg) + 0.5d0 * k1_cavsig2(j, at, fg))
      B(4) = Gamma * (xi ** 2) * (cav2(j, k, at) + 0.5d0 * k1_cav2(j, k, at)) + &
           & -CONJG(gkl(j)) * xi * (cavsig2(k, at, fe) + 0.5d0 * k1_cavsig2(k, at, fe)) + &
           & -CONJG(gkl(k)) * xi * (cavsig2(j, at, fe) + 0.5d0 * k1_cavsig2(j, at, fe))
      B(5) = i * xi * 0.5d0 * Omega * (cav2(j, k, at) + 0.5d0 * k1_cav2(j, k, at)) + &
      ...
      B(6) = -i * xi * 0.5d0 * Omega * (cav2(j, k, at) + 0.5d0 * k1_cav2(j, k, at))
      B(7) = -CONJG(gkl(j)) * (cavsig2(k, at, ef) + 0.5d0 * k1_cavsig2(k, at, ef)) + &
           & -CONJG(gkl(k)) * (cavsig2(j, at, ef) + 0.5d0 * k1_cavsig2(j, at, ef))
      B(8) = 0.0d0
      ! Calculate k2
      k2_cavsig3(j, k, at, :) = dt * (MATMUL(Mat, (cavsig3(j, k, at, :) + 0.5d0 * k1_cavsig3(j, k, at, :))) + B)

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * (cavsig2(k, at, eg) + 0.5d0 * k2_cavsig2(k, at, eg)) + &
           & -CONJG(gkl(k)) * (cavsig2(j, at, eg) + 0.5d0 * k2_cavsig2(j, at, eg))
      B(2) = -CONJG(gkl(j)) * (cavsig2(k, at, ee) + 0.5d0 * k2_cavsig2(k, at, ee)) + &
           & -CONJG(gkl(k)) * (cavsig2(j, at, ee) + 0.5d0 * k2_cavsig2(j, at, ee))
      B(3) = -CONJG(gkl(j)) * xi * (cavsig2(k, at, fg) + 0.5d0 * k2_cavsig2(k, at, fg)) + &
           & -CONJG(gkl(k)) * xi * (cavsig2(j, at, fg) + 0.5d0 * k2_cavsig2(j, at, fg))
      B(4) = Gamma * (xi ** 2) * (cav2(j, k, at) + 0.5d0 * k2_cav2(j, k, at)) + &
           & -CONJG(gkl(j)) * xi * (cavsig2(k, at, fe) + 0.5d0 * k2_cavsig2(k, at, fe)) + &
           & -CONJG(gkl(k)) * xi * (cavsig2(j, at, fe) + 0.5d0 * k2_cavsig2(j, at, fe))
      B(5) = i * xi * 0.5d0 * Omega * (cav2(j, k, at) + 0.5d0 * k2_cav2(j, k, at)) + &
      ...
      B(6) = -i * xi * 0.5d0 * Omega * (cav2(j, k, at) + 0.5d0 * k2_cav2(j, k, at))
      B(7) = -CONJG(gkl(j)) * (cavsig2(k, at, ef) + 0.5d0 * k2_cavsig2(k, at, ef)) + &
           & -CONJG(gkl(k)) * (cavsig2(j, at, ef) + 0.5d0 * k2_cavsig2(j, at, ef))
      B(8) = 0.0d0
      ! Calculate k3
      k3_cavsig3(j, k, at, :) = dt * (MATMUL(Mat, (cavsig3(j, k, at, :) + 0.5d0 * k2_cavsig3(j, k, at, :))) + B)

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * (cavsig2(k, at, eg) + k3_cavsig2(k, at, eg)) + &
           & -CONJG(gkl(k)) * (cavsig2(j, at, eg) + k3_cavsig2(j, at, eg))
      B(2) = -CONJG(gkl(j)) * (cavsig2(k, at, ee) + k3_cavsig2(k, at, ee)) + &
           & -CONJG(gkl(k)) * (cavsig2(j, at, ee) + k3_cavsig2(j, at, ee))
      B(3) = -CONJG(gkl(j)) * xi * (cavsig2(k, at, fg) + k3_cavsig2(k, at, fg)) + &
           & -CONJG(gkl(k)) * xi * (cavsig2(j, at, fg) + k3_cavsig2(j, at, fg))
      B(4) = Gamma * (xi ** 2) * (cav2(j, k, at) + k3_cav2(j, k, at)) + &
           & -CONJG(gkl(j)) * xi * (cavsig2(k, at, fe) + k3_cavsig2(k, at, fe)) + &
           & -CONJG(gkl(k)) * xi * (cavsig2(j, at, fe) + k3_cavsig2(j, at, fe))
      B(5) = i * xi * 0.5d0 * Omega * (cav2(j, k, at) + k3_cav2(j, k, at)) + &
      ...
      B(6) = -i * xi * 0.5d0 * Omega * (cav2(j, k, at) + k3_cav2(j, k, at))
      B(7) = -CONJG(gkl(j)) * (cavsig2(k, at, ef) + k3_cavsig2(k, at, ef)) + &
           & -CONJG(gkl(k)) * (cavsig2(j, at, ef) + k3_cavsig2(j, at, ef))
      B(8) = 0.0d0
      ! Calculate k4
      k4_cavsig3(j, k, at, :) = dt * (MATMUL(Mat, (cavsig3(j, k, at, :) + k3_cavsig3(j, k, at, :))) + B)
      ...
      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * cavsig2(k, a, eg) + &
           & -gkl(k) * cavsig2(j, at, ge)
      B(2) = -CONJG(gkl(j)) * cavsig2(k, a, ee) + &
           & -gkl(k) * xi * cavsig2(j, at, gf)
      B(3) = -CONJG(gkl(j)) * xi * cavsig2(k, a, fg) + &
           & -gkl(k) * cavsig2(j, at, ee)
      B(4) = Gamma * (xi ** 2) * cav2(j, k, ata) + &
           & -CONJG(gkl(j)) * xi * cavsig2(k, a, fe) + &
           & -gkl(k) * xi * cavsig2(j, at, ef)
      B(5) = i * xi * 0.5d0 * Omega * cav2(j, k, ata) + &
           & CONJG(gkl(j)) * xi * cavsig2(k, a, gg) + &
           & CONJG(gkl(j)) * xi * cavsig2(k, a, ee) + &
           & -CONJG(gkl(j)) * xi * cav1(k, a)
      B(6) = -i * xi * 0.5d0 * Omega * cav2(j, k, ata) + &
           & gkl(k) * xi * cavsig2(j, at, gg) + &
           & gkl(k) * xi * cavsig2(j, at, ee) + &
           & -gkl(k) * xi * cav1(j, at)
      B(7) = -CONJG(gkl(j)) * cavsig2(k, a, ef)
      B(8) = - gkl(k) * cavsig2(j, at, fe)
      ! Calculate k1
      k1_cavsig3(j, k, ata, :) = dt * (MATMUL(Mat, cavsig3(j, k, ata, :)) + B)

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * (cavsig2(k, a, eg) + 0.5d0 * k1_cavsig2(k, a, eg)) + &
           & -gkl(k) * (cavsig2(j, at, ge) + 0.5d0 * k1_cavsig2(j, at, ge))
      B(2) = -CONJG(gkl(j)) * (cavsig2(k, a, ee) + 0.5d0 * k1_cavsig2(k, a, ee)) + &
           & -gkl(k) * xi * (cavsig2(j, at, gf) + 0.5d0 * k1_cavsig2(j, at, gf))
      B(3) = -CONJG(gkl(j)) * xi * (cavsig2(k, a, fg) + 0.5d0 * k1_cavsig2(k, a, fg)) + &
           & -gkl(k) * (cavsig2(j, at, ee) + 0.5d0 * k1_cavsig2(j, at, ee))
      B(4) = Gamma * (xi ** 2) * (cav2(j, k, ata) + 0.5d0 * k1_cav2(j, k, ata)) + &
           & -CONJG(gkl(j)) * xi * (cavsig2(k, a, fe) + 0.5d0 * k1_cavsig2(k, a, fe)) + &
           & -gkl(k) * xi * (cavsig2(j, at, ef) + 0.5d0 * k1_cavsig2(j, at, ef))
      B(5) = i * xi * 0.5d0 * Omega * (cav2(j, k, ata) + 0.5d0 * k1_cav2(j, k, ata)) + &
      ...
      B(6) = -i * xi * 0.5d0 * Omega * (cav2(j, k, ata) + 0.5d0 * k1_cav2(j, k, ata)) + &
      ...
      B(7) = -CONJG(gkl(j)) * (cavsig2(k, a, ef) + 0.5d0 * k1_cavsig2(k, a, ef))
      B(8) = - gkl(k) * (cavsig2(j, at, fe) + 0.5d0 * k1_cavsig2(j, at, fe))
      ! Calculate k2
      k2_cavsig3(j, k, ata, :) = dt * (MATMUL(Mat, (cavsig3(j, k, ata, :) + 0.5d0 * k1_cavsig3(j, k, ata, :))) + B)

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * (cavsig2(k, a, eg) + 0.5d0 * k2_cavsig2(k, a, eg)) + &
           & -gkl(k) * (cavsig2(j, at, ge) + 0.5d0 * k2_cavsig2(j, at, ge))
      B(2) = -CONJG(gkl(j)) * (cavsig2(k, a, ee) + 0.5d0 * k2_cavsig2(k, a, ee)) + &
           & -gkl(k) * xi * (cavsig2(j, at, gf) + 0.5d0 * k2_cavsig2(j, at, gf))
      B(3) = -CONJG(gkl(j)) * xi * (cavsig2(k, a, fg) + 0.5d0 * k2_cavsig2(k, a, fg)) + &
           & -gkl(k) * (cavsig2(j, at, ee) + 0.5d0 * k2_cavsig2(j, at, ee))
      B(4) = Gamma * (xi ** 2) * (cav2(j, k, ata) + 0.5d0 * k2_cav2(j, k, ata)) + &
           & -CONJG(gkl(j)) * xi * (cavsig2(k, a, fe) + 0.5d0 * k2_cavsig2(k, a, fe)) + &
           & -gkl(k) * xi * (cavsig2(j, at, ef) + 0.5d0 * k2_cavsig2(j, at, ef))
      B(5) = i * xi * 0.5d0 * Omega * (cav2(j, k, ata) + 0.5d0 * k2_cav2(j, k, ata)) + &
      ...
      B(6) = -i * xi * 0.5d0 * Omega * (cav2(j, k, ata) + 0.5d0 * k2_cav2(j, k, ata)) + &
      ...
      B(7) = -CONJG(gkl(j)) * (cavsig2(k, a, ef) + 0.5d0 * k2_cavsig2(k, a, ef))
      B(8) = - gkl(k) * (cavsig2(j, at, fe) + 0.5d0 * k2_cavsig2(j, at, fe))
      ! Calculate k3
      k3_cavsig3(j, k, ata, :) = dt * (MATMUL(Mat, (cavsig3(j, k, ata, :) + 0.5d0 * k2_cavsig3(j, k, ata, :))) + B)

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * (cavsig2(k, a, eg) + k3_cavsig2(k, a, eg)) + &
           & -gkl(k) * (cavsig2(j, at, ge) + k3_cavsig2(j, at, ge))
      B(2) = -CONJG(gkl(j)) * (cavsig2(k, a, ee) + k3_cavsig2(k, a, ee)) + &
           & -gkl(k) * xi * (cavsig2(j, at, gf) + k3_cavsig2(j, at, gf))
      B(3) = -CONJG(gkl(j)) * xi * (cavsig2(k, a, fg) + k3_cavsig2(k, a, fg)) + &
           & -gkl(k) * (cavsig2(j, at, ee) + k3_cavsig2(j, at, ee))
      B(4) = Gamma * (xi ** 2) * (cav2(j, k, ata) + k3_cav2(j, k, ata)) + &
           & -CONJG(gkl(j)) * xi * (cavsig2(k, a, fe) + k3_cavsig2(k, a, fe)) + &
           & -gkl(k) * xi * (cavsig2(j, at, ef) + k3_cavsig2(j, at, ef))
      B(5) = i * xi * 0.5d0 * Omega * (cav2(j, k, ata) + k3_cav2(j, k, ata)) + &
      ...
      B(6) = -i * xi * 0.5d0 * Omega * (cav2(j, k, ata) + k3_cav2(j, k, ata)) + &
           & gkl(k) * xi * (cavsig2(j, at, gg) + k3_cavsig2(j, at, gg)) + &
           & gkl(k) * xi * (cavsig2(j, at, ee) + k3_cavsig2(j, at, ee)) + &
           & -gkl(k) * xi * (cav1(j, at) + k3_cav1(j, at))
      B(7) = -CONJG(gkl(j)) * (cavsig2(k, a, ef) + k3_cavsig2(k, a, ef))
      B(8) = - gkl(k) * (cavsig2(j, at, fe) + k3_cavsig2(j, at, fe))
      ! Calculate k4
      k4_cavsig3(j, k, ata, :) = dt * (MATMUL(Mat, (cavsig3(j, k, ata, :) + k3_cavsig3(j, k, ata, :))) + B)
  ...
  DO l = -N, N
    DO k = -N, N
      DO j = -N, N
        ...
        B(5) = i * xi * 0.5d0 * Omega * (cav3(j, k, l, a) + 0.5d0 * k1_cav3(j, k, l, a)) + &
        ...
        B(6) = -i * xi * 0.5d0 * Omega * (cav3(j, k, l, a) + 0.5d0 * k1_cav3(j, k, l, a)) + &
        ...
        B(7) = -CONJG(gkl(j)) * (cavsig3(k, l, a, ef) + 0.5d0 * k1_cavsig3(k, l, a, ef))
        B(8) = -gkl(k) * (cavsig3(j, l, ata, fe) + 0.5d0 * k1_cavsig3(j, l, ata, fe)) + &
             & -gkl(l) * (cavsig3(j, k, ata, fe) + 0.5d0 * k1_cavsig3(j, k, ata, fe))
        ! Calculate k2
        k2_cavsig4(j, k, l, a, :) = dt * (MATMUL(Mat, (cavsig4(j, k, l, a, :) + 0.5d0 * k1_cavsig4(j, k, l, a, :))) + B)

        ! Set the non-homogeneous vector
        B = 0.0d0
        B(1) = -CONJG(gkl(j)) * (cavsig3(k, l, a, eg) + 0.5d0 * k2_cavsig3(k, l, a, eg)) + &
             & -gkl(k) * (cavsig3(j, l, ata, ge) + 0.5d0 * k2_cavsig3(j, l, ata, ge)) + &
             & -gkl(l) * (cavsig3(j, k, ata, ge) + 0.5d0 * k2_cavsig3(j, k, ata, ge))
        B(2) = -CONJG(gkl(j)) * (cavsig3(k, l, a, ee) + 0.5d0 * k2_cavsig3(k, l, a, ee)) + &
             & -gkl(k) * xi * (cavsig3(j, l, ata, gf) + 0.5d0 * k2_cavsig3(j, l, ata, gf)) + &
             & -gkl(l) * xi * (cavsig3(j, k, ata, gf) + 0.5d0 * k2_cavsig3(j, k, ata, gf))
        B(3) = -CONJG(gkl(j)) * xi * (cavsig3(k, l, a, fg) + 0.5d0 * k2_cavsig3(k, l, a, fg)) + &
             & -gkl(k) * (cavsig3(j, l, ata, ee) + 0.5d0 * k2_cavsig3(j, l, ata, ee)) + &
             & -gkl(l) * (cavsig3(j, k, ata, ee) + 0.5d0 * k2_cavsig3(j, k, ata, ee))
        B(4) = Gamma * (xi ** 2) * (cav3(j, k, l, a) + 0.5d0 * k2_cav3(j, k, l, a)) + &
        ...
        B(5) = i * xi * 0.5d0 * Omega * (cav3(j, k, l, a) + 0.5d0 * k2_cav3(j, k, l, a)) + &
        ...
        B(6) = -i * xi * 0.5d0 * Omega * (cav3(j, k, l, a) + 0.5d0 * k2_cav3(j, k, l, a)) + &
        ...
        B(7) = -CONJG(gkl(j)) * (cavsig3(k, l, a, ef) + 0.5d0 * k2_cavsig3(k, l, a, ef))
        B(8) = -gkl(k) * (cavsig3(j, l, ata, fe) + 0.5d0 * k2_cavsig3(j, l, ata, fe)) + &
             & -gkl(l) * (cavsig3(j, k, ata, fe) + 0.5d0 * k2_cavsig3(j, k, ata, fe))
        ! Calculate k3
        k3_cavsig4(j, k, l, a, :) = dt * (MATMUL(Mat, (cavsig4(j, k, l, a, :) + 0.5d0 * k2_cavsig4(j, k, l, a, :))) + B)

        ! Set the non-homogeneous vector
        B = 0.0d0
        B(1) = -CONJG(gkl(j)) * (cavsig3(k, l, a, eg) + k3_cavsig3(k, l, a, eg)) + &
             & -gkl(k) * (cavsig3(j, l, ata, ge) + k3_cavsig3(j, l, ata, ge)) + &
             & -gkl(l) * (cavsig3(j, k, ata, ge) + k3_cavsig3(j, k, ata, ge))
        B(2) = -CONJG(gkl(j)) * (cavsig3(k, l, a, ee) + k3_cavsig3(k, l, a, ee)) + &
             & -gkl(k) * xi * (cavsig3(j, l, ata, gf) + k3_cavsig3(j, l, ata, gf)) + &
             & -gkl(l) * xi * (cavsig3(j, k, ata, gf) + k3_cavsig3(j, k, ata, gf))
        B(3) = -CONJG(gkl(j)) * xi * (cavsig3(k, l, a, fg) + k3_cavsig3(k, l, a, fg)) + &
             & -gkl(k) * (cavsig3(j, l, ata, ee) + k3_cavsig3(j, l, ata, ee)) + &
             & -gkl(l) * (cavsig3(j, k, ata, ee) + k3_cavsig3(j, k, ata, ee))
        B(4) = Gamma * (xi ** 2) * (cav3(j, k, l, a) + k3_cav3(j, k, l, a)) + &
        ...
        B(5) = i * xi * 0.5d0 * Omega * (cav3(j, k, l, a) + k3_cav3(j, k, l, a)) + &
        ...
        B(6) = -i * xi * 0.5d0 * Omega * (cav3(j, k, l, a) + k3_cav3(j, k, l, a)) + &
        ...
        B(7) = -CONJG(gkl(j)) * (cavsig3(k, l, a, ef) + k3_cavsig3(k, l, a, ef))
        B(8) = -gkl(k) * (cavsig3(j, l, ata, fe) + k3_cavsig3(j, l, ata, fe)) + &
             & -gkl(l) * (cavsig3(j, k, ata, fe) + k3_cavsig3(j, k, ata, fe))
        ! Calculate k4
        k4_cavsig4(j, k, l, a, :) = dt * (MATMUL(Mat, (cavsig4(j, k, l, a, :) + k3_cavsig4(j, k, l, a, :))) + B)
        ...
        B(5) = i * xi * 0.5d0 * Omega * (cav3(j, k, l, at) + 0.5d0 * k1_cav3(j, k, l, at)) + &
        ...
        B(6) = -i * xi * 0.5d0 * Omega * (cav3(j, k, l, at) + 0.5d0 * k1_cav3(j, k, l, at)) + &
        ...
        B(7) = -CONJG(gkl(j)) * (cavsig3(k, l, ata, ef) + 0.5d0 * k1_cavsig3(k, l, ata, ef)) + &
             & -CONJG(gkl(k)) * (cavsig3(j, l, ata, ef) + 0.5d0 * k1_cavsig3(j, l, ata, ef))
        B(8) = -gkl(l) * (cavsig3(j, k, at, fe) + 0.5d0 * k1_cavsig3(j, k, at, fe))
        ! Calculate k2
        k2_cavsig4(j, k, l, at, :) = dt * (MATMUL(Mat, (cavsig4(j, k, l, at, :) + 0.5d0 * k1_cavsig4(j, k, l, at, :))) + B)

        ! Set the non-homogeneous vector
        B = 0.0d0
        B(1) = -CONJG(gkl(j)) * (cavsig3(k, l, ata, eg) + 0.5d0 * k2_cavsig3(k, l, ata, eg)) + &
             & -CONJG(gkl(k)) * (cavsig3(j, l, ata, eg) + 0.5d0 * k2_cavsig3(j, l, ata, eg)) + &
             & -gkl(l) * (cavsig3(j, k, at, ge) + 0.5d0 * k2_cavsig3(j, k, at, ge))
        B(2) = -CONJG(gkl(j)) * (cavsig3(k, l, ata, ee) + 0.5d0 * k2_cavsig3(k, l, ata, ee)) + &
             & -CONJG(gkl(k)) * (cavsig3(j, l, ata, ee) + 0.5d0 * k2_cavsig3(j, l, ata, ee)) + &
             & -gkl(l) * xi * (cavsig3(j, k, at, gf) + 0.5d0 * k2_cavsig3(j, k, at, gf))
        B(3) = -CONJG(gkl(j)) * xi * (cavsig3(k, l, ata, fg) + 0.5d0 * k2_cavsig3(k, l, ata, fg)) + &
             & -CONJG(gkl(k)) * xi * (cavsig3(j, l, ata, fg) + 0.5d0 * k2_cavsig3(j, l, ata, fg)) + &
             & -gkl(l) * (cavsig3(j, k, at, ee) + 0.5d0 * k2_cavsig3(j, k, at, ee))
        B(4) = Gamma * (xi ** 2) * (cav3(j, k, l, at) + 0.5d0 * k2_cav3(j, k, l, at)) + &
        ...
        B(5) = i * xi * 0.5d0 * Omega * (cav3(j, k, l, at) + 0.5d0 * k2_cav3(j, k, l, at)) + &
        ...
        B(6) = -i * xi * 0.5d0 * Omega * (cav3(j, k, l, at) + 0.5d0 * k2_cav3(j, k, l, at)) + &
        ...
        B(7) = -CONJG(gkl(j)) * (cavsig3(k, l, ata, ef) + 0.5d0 * k2_cavsig3(k, l, ata, ef)) + &
             & -CONJG(gkl(k)) * (cavsig3(j, l, ata, ef) + 0.5d0 * k2_cavsig3(j, l, ata, ef))
        B(8) = -gkl(l) * (cavsig3(j, k, at, fe) + 0.5d0 * k2_cavsig3(j, k, at, fe))
        ! Calculate k3
        k3_cavsig4(j, k, l, at, :) = dt * (MATMUL(Mat, (cavsig4(j, k, l, at, :) + 0.5d0 * k2_cavsig4(j, k, l, at, :))) + B)

        ! Set the non-homogeneous vector
        B = 0.0d0
        B(1) = -CONJG(gkl(j)) * (cavsig3(k, l, ata, eg) + k3_cavsig3(k, l, ata, eg)) + &
             & -CONJG(gkl(k)) * (cavsig3(j, l, ata, eg) + k3_cavsig3(j, l, ata, eg)) + &
             & -gkl(l) * (cavsig3(j, k, at, ge) + k3_cavsig3(j, k, at, ge))
        B(2) = -CONJG(gkl(j)) * (cavsig3(k, l, ata, ee) + k3_cavsig3(k, l, ata, ee)) + &
             & -CONJG(gkl(k)) * (cavsig3(j, l, ata, ee) + k3_cavsig3(j, l, ata, ee)) + &
             & -gkl(l) * xi * (cavsig3(j, k, at, gf) + k3_cavsig3(j, k, at, gf))
        B(3) = -CONJG(gkl(j)) * xi * (cavsig3(k, l, ata, fg) + k3_cavsig3(k, l, ata, fg)) + &
             & -CONJG(gkl(k)) * xi * (cavsig3(j, l, ata, fg) + k3_cavsig3(j, l, ata, fg)) + &
             & -gkl(l) * (cavsig3(j, k, at, ee) + k3_cavsig3(j, k, at, ee))
        B(4) = Gamma * (xi ** 2) * (cav3(j, k, l, at) + k3_cav3(j, k, l, at)) + &
        ...
        B(5) = i * xi * 0.5d0 * Omega * (cav3(j, k, l, at) + k3_cav3(j, k, l, at)) + &
        ...
        B(6) = -i * xi * 0.5d0 * Omega * (cav3(j, k, l, at) + k3_cav3(j, k, l, at)) + &
        ...
        B(7) = -CONJG(gkl(j)) * (cavsig3(k, l, ata, ef) + k3_cavsig3(k, l, ata, ef)) + &
             & -CONJG(gkl(k)) * (cavsig3(j, l, ata, ef) + k3_cavsig3(j, l, ata, ef))
        B(8) = -gkl(l) * (cavsig3(j, k, at, fe) + k3_cavsig3(j, k, at, fe))
        ! Calculate k4
        k4_cavsig4(j, k, l, at, :) = dt * (MATMUL(Mat, (cavsig4(j, k, l, at, :) + k3_cavsig4(j, k, l, at, :))) + B)

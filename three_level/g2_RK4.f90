...
DO j = -N, N
  ...
  B = 0.0d0
  B(1) = -gkl(j) * sigma_ss(ge)
  B(2) = -gkl(j) * xi * sigma_ss(gf)
  B(3) = -gkl(j) * sigma_ss(ee)
  B(4) = Gamma * (xi ** 2) * cav1_ss(j, a) + &
       & -gkl(j) * xi * sigma_ss(ef)
  B(5) = i * xi * 0.5d0 * Omega * cav1_ss(j, a)
  B(6) = -i * xi * 0.5d0 * Omega * cav1_ss(j, a) + &
       & gkl(j) * xi * sigma_ss(gg) + &
       & gkl(j) * xi * sigma_ss(ee) + &
       & -gkl(j) * xi
  B(7) = 0.0d0
  B(8) = -gkl(j) * sigma_ss(fe)
  ...
  ! Set the non-homogeneous vector
  B = 0.0d0
  B(1) = -CONJG(gkl(j)) * sigma_ss(eg)
  B(2) = -CONJG(gkl(j)) * sigma_ss(ee)
  B(3) = -CONJG(gkl(j)) * xi * sigma_ss(fg)
  B(4) = Gamma * (xi ** 2) * cav1_ss(j, at) + &
       & -CONJG(gkl(j)) * xi * sigma_ss(fe)
  B(5) = i * xi * 0.5d0 * Omega * cav1_ss(j, at) + &
       & CONJG(gkl(j)) * xi * sigma_ss(gg) + &
       & CONJG(gkl(j)) * xi * sigma_ss(ee) + &
       & -CONJG(gkl(j)) * xi
  B(6) = -i * xi * 0.5d0 * Omega * cav1_ss(j, at)
  B(7) = -CONJG(gkl(j)) * sigma_ss(ef)
  B(8) = 0.0d0
...
DO k = -N, N
  DO j = -N, N
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -gkl(j) * cavsig2_ss(k, a, ge) + &
         & -gkl(k) * cavsig2_ss(j, a, ge)
    B(2) = -gkl(j) * xi * cavsig2_ss(k, a, gf) + &
         & -gkl(k) * xi * cavsig2_ss(j, a, gf)
    B(3) = -gkl(j) * cavsig2_ss(k, a, ee) + &
         & -gkl(k) * cavsig2_ss(j, a, ee)
    B(4) = Gamma * (xi ** 2) * cav2_ss(j, k, a) + &
         & -gkl(j) * xi * cavsig2_ss(k, a, ef) + &
         & -gkl(k) * xi * cavsig2_ss(j, a, ef)
    B(5) = i * xi * 0.5d0 * Omega * cav2_ss(j, k, a)
    B(6) = -i * xi * 0.5d0 * Omega * cav2_ss(j, k, a) + &
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -CONJG(gkl(j)) * cavsig2_ss(k, at, eg) + &
         & -CONJG(gkl(k)) * cavsig2_ss(j, at, eg)
    B(2) = -CONJG(gkl(j)) * cavsig2_ss(k, at, ee) + &
         & -CONJG(gkl(k)) * cavsig2_ss(j, at, ee)
    B(3) = -CONJG(gkl(j)) * xi * cavsig2_ss(k, at, fg) + &
         & -CONJG(gkl(k)) * xi * cavsig2_ss(j, at, fg)
    B(4) = Gamma * (xi ** 2) * cav2_ss(j, k, at) + &
         & -CONJG(gkl(j)) * xi * cavsig2_ss(k, at, fe) + &
         & -CONJG(gkl(k)) * xi * cavsig2_ss(j, at, fe)
    B(5) = i * xi * 0.5d0 * Omega * cav2_ss(j, k, at) + &
    ...
    B(6) = -i * xi * 0.5d0 * Omega * cav2_ss(j, k, at)
    B(7) = -CONJG(gkl(j)) * cavsig2_ss(k, at, ef) + &
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -CONJG(gkl(j)) * cavsig2_ss(k, a, eg) + &
         & -gkl(k) * cavsig2_ss(j, at, ge)
    B(2) = -CONJG(gkl(j)) * cavsig2_ss(k, a, ee) + &
         & -gkl(k) * xi * cavsig2_ss(j, at, gf)
    B(3) = -CONJG(gkl(j)) * xi * cavsig2_ss(k, a, fg) + &
         & -gkl(k) * cavsig2_ss(j, at, ee)
    B(4) = Gamma * (xi ** 2) * cav2_ss(j, k, ata) + &
         & -CONJG(gkl(j)) * xi * cavsig2_ss(k, a, fe) + &
         & -gkl(k) * xi * cavsig2_ss(j, at, ef)
    B(5) = i * xi * 0.5d0 * Omega * cav2_ss(j, k, ata) + &
         & CONJG(gkl(j)) * xi * cavsig2_ss(k, a, gg) + &
         & CONJG(gkl(j)) * xi * cavsig2_ss(k, a, ee) + &
         & -CONJG(gkl(j)) * xi * cav1_ss(k, a)
    B(6) = -i * xi * 0.5d0 * Omega * cav2_ss(j, k, ata) + &
         & gkl(k) * xi * cavsig2_ss(j, at, gg) + &
         & gkl(k) * xi * cavsig2_ss(j, at, ee) + &
         & -gkl(k) * xi * cav1_ss(j, at)
    B(7) = -CONJG(gkl(j)) * cavsig2_ss(k, a, ef)
    B(8) = - gkl(k) * cavsig2_ss(j, at, fe)
...
DO l = -N, N
  DO k = -N, N
    DO j = -N, N
      ...
      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * cavsig3_ss(k, l, a, eg) + &
           & -gkl(k) * cavsig3_ss(j, l, ata, ge) + &
           & -gkl(l) * cavsig3_ss(j, k, ata, ge)
      B(2) = -CONJG(gkl(j)) * cavsig3_ss(k, l, a, ee) + &
           & -gkl(k) * xi * cavsig3_ss(j, l, ata, gf) + &
           & -gkl(l) * xi * cavsig3_ss(j, k, ata, gf)
      B(3) = -CONJG(gkl(j)) * xi * cavsig3_ss(k, l, a, fg) + &
           & -gkl(k) * cavsig3_ss(j, l, ata, ee) + &
           & -gkl(l) * cavsig3_ss(j, k, ata, ee)
      B(4) = Gamma * (xi ** 2) * cav3_ss(j, k, l, a) + &
           & -CONJG(gkl(j)) * xi * cavsig3_ss(k, l, a, fe) + &
           & -gkl(k) * xi * cavsig3_ss(j, l, ata, ef) + &
           & -gkl(l) * xi * cavsig3_ss(j, k, ata, ef)
      B(5) = i * xi * 0.5d0 * Omega * cav3_ss(j, k, l, a) + &
           & CONJG(gkl(j)) * xi * cavsig3_ss(k, l, a, gg) + &
           & CONJG(gkl(j)) * xi * cavsig3_ss(k, l, a, ee) + &
           & -CONJG(gkl(j)) * xi * cav2_ss(k, l, a)
      B(6) = -i * xi * 0.5d0 * Omega * cav3_ss(j, k, l, a) + &
      ...
      B(7) = -CONJG(gkl(j)) * cavsig3_ss(k, l, a, ef)
      B(8) = -gkl(k) * cavsig3_ss(j, l, ata, fe) + &
      ...
      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -CONJG(gkl(j)) * cavsig3_ss(k, l, ata, eg) + &
           & -CONJG(gkl(k)) * cavsig3_ss(j, l, ata, eg) + &
           & -gkl(l) * cavsig3_ss(j, k, at, ge)
      B(2) = -CONJG(gkl(j)) * cavsig3_ss(k, l, ata, ee) + &
           & -CONJG(gkl(k)) * cavsig3_ss(j, l, ata, ee) + &
           & -gkl(l) * xi * cavsig3_ss(j, k, at, gf)
      B(3) = -CONJG(gkl(j)) * xi * cavsig3_ss(k, l, ata, fg) + &
           & -CONJG(gkl(k)) * xi * cavsig3_ss(j, l, ata, fg) + &
           & -gkl(l) * cavsig3_ss(j, k, at, ee)
      B(4) = Gamma * (xi ** 2) * cav3_ss(j, k, l, at) + &
           & -CONJG(gkl(j)) * xi * cavsig3_ss(k, l, ata, fe) + &
           & -CONJG(gkl(k)) * xi * cavsig3_ss(j, l, ata, fe) + &
           & -gkl(l) * xi * cavsig3_ss(j, k, at, ef)
      B(5) = i * xi * 0.5d0 * Omega * cav3_ss(j, k, l, at) + &

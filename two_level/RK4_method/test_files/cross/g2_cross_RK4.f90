...
! Steady states
ALLOCATE(cav1_ss(-N:N, 4)); cav1_ss = 0.0d0
! Second-order: Cavity and Atom
ALLOCATE(cavsig2_ss(-N:N, 4, N_mat)); cavsig2_ss = 0.0d0
! Second-order: Cavity
...
DO j = -N, N
  ...
  ! < b_{j} > !
  !-----------!
  cav1_ss(j, b) = -gkl(j, b) * sigma_ss(sm) / &
                & (kappa(b) + i * wl(j, b))
  ...
  ! < a^{\dagger}_{j} > !
  !---------------------!
  cav1_ss(j, bt) = -CONJG(gkl(j, b)) * sigma_ss(sp) / &
                 & (kappa(b) - i * wl(j, b))
  ...
  ! Calculate steady state
  cavsig2_ss(j, b, :) = -MATMUL(Mat_inv, B_vec)
  ...
  ! Calculate steady state
  cavsig2_ss(j, bt, :) = -MATMUL(Mat_inv, B_vec)
...
DO k = -N, N
  DO j = -N, N
    ...
    ! < a_{j} b_{k} > !
    !-----------------!
    cav2_ss(j, k, ab) = -gkl(j, a) * cavsig2_ss(k, b, sm) + &
    ...
    !-------------------------------------!
    cav2_ss(j, k, atbt) = -CONJG(gkl(j, a)) * cavsig2_ss(k, bt, sp) !+ &
    ...
    ! ! < b^{\dagger}_{j} b_{k} > !
    ! !---------------------------!
    ! cav2_ss(j, k, btb) = -CONJG(gkl(j, b)) * cavsig2_ss(k, b, sp) + &
    !                    & -gkl(k, b) * cavsig2_ss(j, bt, sm)
    ...
    ! ! < a^{\dagger}_{j} b_{k} > !
    ! !---------------------------!
    ! cav2_ss(j, k, atb) = -CONJG(gkl(j, a)) * cavsig2_ss(k, b, sp) + &
    ...
    ! B_vec = 0.0d0
    ! B_vec(1) = 0.0d0
    ! B_vec(2) = -0.5d0 * gkl(j, a) * cavsig2_ss(k, b, sz) + &
    !          & -0.5d0 * gkl(j, a) * cav1_ss(k, b) + &
    !          & -0.5d0 * gkl(k, b) * cavsig2_ss(j, a, sz) + &
    !          & -0.5d0 * gkl(k, b) * cav1_ss(j, a)
    ! B_vec(3) = -gamma * cav2_ss(j, k, ab) + &
    !          & gkl(j, a) * cavsig2_ss(k, b, sm) + &
    !          & gkl(k, b) * cavsig2_ss(j, a, sm)
    !
    ! ! Set inverse matrix
    ! Mat_inv = Mat
    ...
    ! ! Set the non-homogeneous vector
    ! B_vec = 0.0d0
    ! B_vec(1) = -0.5d0 * CONJG(gkl(j, a)) * cavsig2_ss(k, bt, sz) + &
    !          & -0.5d0 * CONJG(gkl(j, a)) * cav1_ss(k, bt) + &
    !          & -0.5d0 * CONJG(gkl(k, b)) * cavsig2_ss(j, at, sz) + &
    !          & -0.5d0 * CONJG(gkl(k, b)) * cav1_ss(j, at)
    ! B_vec(2) = 0.0d0
    ! B_vec(3) = -gamma * cav2_ss(j, k, atbt) + &
    !          & CONJG(gkl(j, a)) * cavsig2_ss(k, bt, sp) + &
    !          & CONJG(gkl(k, b)) * cavsig2_ss(j, at, sp)
    !
    ! ! Set inverse matrix
    ! Mat_inv = Mat
    ...
    ! ! Set the non-homogeneous vector
    ! B_vec = 0.0d0
    ! B_vec(1) = -0.5d0 * CONJG(gkl(j, b)) * cavsig2_ss(k, b, sz) + &
    !          & -0.5d0 * CONJG(gkl(j, b)) * cav1_ss(k, b)
    ! B_vec(2) = -0.5d0 * gkl(k, b) * cavsig2_ss(j, bt, sz) + &
    !          & -0.5d0 * gkl(k, b) * cav1_ss(j, bt)
    ! B_vec(3) = -gamma * cav2_ss(j, k, btb) + &
    !          & CONJG(gkl(j, b)) * cavsig2_ss(k, b, sp) + &
    !          & gkl(k, b) * cavsig2_ss(j, bt, sm)
    !
    ! ! Set inverse matrix
    ! Mat_inv = Mat
    ...
    ! ! Set the non-homogeneous vector
    ! B_vec = 0.0d0
    ! B_vec(1) = -0.5d0 * CONJG(gkl(j, a)) * cavsig2_ss(k, b, sz) + &
    !          & -0.5d0 * CONJG(gkl(j, a)) * cav1_ss(k, b)
    ! B_vec(2) = -0.5d0 * gkl(k, b) * cavsig2_ss(j, at, sz) + &
    !          & -0.5d0 * gkl(k, b) * cav1_ss(j, at)
    ! B_vec(3) = -gamma * cav2_ss(j, k, atb) + &
    !          & CONJG(gkl(j, a)) * cavsig2_ss(k, b, sp) + &
    !          & gkl(k, b) * cavsig2_ss(j, at, sm)
    !
    ! ! Set inverse matrix
    ! Mat_inv = Mat
    ...
    ! B_vec(2) = -0.5d0 * gkl(k, a) * cavsig2_ss(j, bt, sz) + &
    !          & -0.5d0 * gkl(k, a) * cav1_ss(j, bt)
...
DO k = -N, N
  DO j = -N, N
    ...
    photonb_ss = photonb_ss + cav2_ss(j, k, btb)
  END DO
END DO
...
! Reset at variable for cavsig2 and cav2 arrays
at = 2

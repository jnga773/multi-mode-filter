...
! Integer indices for: ab, a^{\dagger} b^{\dagger}, b^{\dagger} b
INTEGER, PARAMETER                                        :: ab = 1, atbt = 2, btb = 4
...
DO cj = 1, 2
  ...
  DO j = -N, N
    ...
    B_vec = 0.0d0
    B_vec(1) = 0.0d0
    B_vec(2) = -0.5d0 * gkl(j, cj) * (sigma_ss(sz) + 1.0d0)
    B_vec(3) = -gamma * cav1_ss(j, a, cj) + &
    ...
    ! Set the non-homogeneous vector
    B_vec = 0.0d0
    B_vec(1) = -0.5d0 * CONJG(gkl(j, cj)) * (sigma_ss(sz) + 1.0d0)
    B_vec(2) = 0.0d0
    B_vec(3) = -gamma * cav1_ss(j, at, cj) + &
  ...
  DO cj = 1, 2
    ...
    DO k = -N, N
      DO j = -N, N
        ...
        ! Set the non-homogeneous vector
        B_vec = 0.0d0
        B_vec(1) = -0.5d0 * CONJG(gkl(j, cj)) * cavsig2_ss(k, a, sz, ck) + &
             & -0.5d0 * CONJG(gkl(j, cj)) * cav1_ss(k, a, ck)
        B_vec(2) = -0.5d0 * gkl(k, ck) * cavsig2_ss(j, at, sz, cj) + &
             & -0.5d0 * gkl(k, ck) * cav1_ss(j, at, cj)
        B_vec(3) = -gamma * cav2_ss(j, k, ata, cj, ck) + &
...
DO l = -N, N
  DO k = -N, N
    DO j = -N, N
      ...
      !---------------------------------!
      cav3_ss(j, k, l, atab) = -CONJG(gkl(j, a)) * cavsig3_ss(k, l, a, sp, 1, 2) + &
                             & -gkl(k, a) * cavsig3_ss(j, l, ata, sm, 1, 2) + &
                             & -gkl(l, b) * cavsig3_ss(j, k, ata, sm, 1, 1)
      ...
      !---------------------------------!
      cav3_ss(j, k, l, btba) = -CONJG(gkl(j, b)) * cavsig3_ss(k, l, a, sp, 2, 1) + &
                             & -gkl(k, b) * cavsig3_ss(j, l, ata, sm, 2, 1) + &
                             & -gkl(l, a) * cavsig3_ss(j, k, ata, sm, 2, 2)
      ...
      !-------------------------------------------!
      cav3_ss(j, k, l, btata) = -CONJG(gkl(j, b)) * cavsig3_ss(k, l, ata, sp, 1, 1) + &
                              & -CONJG(gkl(k, a)) * cavsig3_ss(j, l, ata, sp, 2, 1) + &
                              & -gkl(l, a) * cavsig3_ss(j, k, at, sm, 2, 1)
      ...
      !-------------------------------------------!
      cav3_ss(j, k, l, atbtb) = -CONJG(gkl(j, a)) * cavsig3_ss(k, l, ata, sp, 2, 2) + &
                              & -CONJG(gkl(k, b)) * cavsig3_ss(j, l, ata, sp, 1, 2) + &
                              & -gkl(l, b) * cavsig3_ss(j, k, at, sm, 1, 2)
      ...
      ! Set the non-homogeneous vector
      B_vec = 0.0d0
      B_vec(1) = -0.5d0 * CONJG(gkl(j, a)) * cavsig3_ss(k, l, a, sz, 1, 2) + &
               & -0.5d0 * CONJG(gkl(j, a)) * cav2_ss(k, l, a, 1, 2)
      B_vec(2) = -0.5d0 * gkl(k, a) * cavsig3_ss(j, l, ata, sz, 1, 2) + &
               & -0.5d0 * gkl(k, a) * cav2_ss(j, l, ata, 1, 2) + &
               & -0.5d0 * gkl(l, b) * cavsig3_ss(j, k, ata, sz, 1, 1) + &
               & -0.5d0 * gkl(l, b) * cav2_ss(j, k, ata, 1, 1)
      B_vec(3) = -gamma * cav3_ss(j, k, l, atab) + &
               & CONJG(gkl(j, a)) * cavsig3_ss(k, l, a, sp, 1, 2) + &
               & gkl(k, a) * cavsig3_ss(j, l, ata, sm, 1, 2) + &
               & gkl(l, b) * cavsig3_ss(j, k, ata, sm, 1, 1)
      ...
      ! Set the non-homogeneous vector
      B_vec = 0.0d0
      B_vec(1) = -0.5d0 * CONJG(gkl(j, b)) * cavsig3_ss(k, l, a, sz, 2, 1) + &
               & -0.5d0 * CONJG(gkl(j, b)) * cav2_ss(k, l, a, 2, 1)
      B_vec(2) = -0.5d0 * gkl(k, b) * cavsig3_ss(j, l, ata, sz, 2, 1) + &
               & -0.5d0 * gkl(k, b) * cav2_ss(j, l, ata, 2, 1) + &
               & -0.5d0 * gkl(l, a) * cavsig3_ss(j, k, ata, sz, 2, 2) + &
               & -0.5d0 * gkl(l, a) * cav2_ss(j, k, ata, 2, 2)
      B_vec(3) = -gamma * cav3_ss(j, k, l, btba) + &
               & CONJG(gkl(j, b)) * cavsig3_ss(k, l, a, sp, 2, 1) + &
               & gkl(k, b) * cavsig3_ss(j, l, ata, sm, 2, 1) + &
               & gkl(l, a) * cavsig3_ss(j, k, ata, sm, 2, 2)
      ...
      ! Set the non-homogeneous vector
      B_vec = 0.0d0
      B_vec(1) = -0.5d0 * CONJG(gkl(j, b)) * cavsig3_ss(k, l, ata, sz, 1, 1) + &
               & -0.5d0 * CONJG(gkl(j, b)) * cav2_ss(k, l, ata, 1, 1) + &
               & -0.5d0 * CONJG(gkl(k, a)) * cavsig3_ss(j, l, ata, sz, 2, 1) + &
               & -0.5d0 * CONJG(gkl(k, a)) * cav2_ss(j, l, ata, 2, 1)
      B_vec(2) = -0.5d0 * gkl(l, a) * cavsig3_ss(j, k, at, sz, 2, 1) + &
               & -0.5d0 * gkl(l, a) * cav2_ss(j, k, at, 2, 1)
      B_vec(3) = -gamma * cav3_ss(j, k, l, btata) + &
               & CONJG(gkl(j, b)) * cavsig3_ss(k, l, ata, sp, 1, 1) + &
               & CONJG(gkl(k, a)) * cavsig3_ss(j, l, ata, sp, 2, 1) + &
               & gkl(l, a) * cavsig3_ss(j, k, at, sm, 2, 1)
      ...
      ! Set the non-homogeneous vector
      B_vec = 0.0d0
      B_vec(1) = -0.5d0 * CONJG(gkl(j, a)) * cavsig3_ss(k, l, ata, sz, 2, 2) + &
               & -0.5d0 * CONJG(gkl(j, a)) * cav2_ss(k, l, ata, 2, 2) + &
               & -0.5d0 * CONJG(gkl(k, b)) * cavsig3_ss(j, l, ata, sz, 1, 2) + &
               & -0.5d0 * CONJG(gkl(k, b)) * cav2_ss(j, l, ata, 1, 2)
      B_vec(2) = -0.5d0 * gkl(l, b) * cavsig3_ss(j, k, at, sz, 1, 2) + &
               & -0.5d0 * gkl(l, b) * cav2_ss(j, k, at, 1, 2)
      B_vec(3) = -gamma * cav3_ss(j, k, l, atbtb) + &
               & CONJG(gkl(j, a)) * cavsig3_ss(k, l, ata, sp, 2, 2) + &
               & CONJG(gkl(k, b)) * cavsig3_ss(j, l, ata, sp, 1, 2) + &
               & gkl(l, b) * cavsig3_ss(j, k, at, sm, 1, 2)
...
DO t = 0, tau_steps
  ...
  DO j = -N, N
    ...
    B_vec = 0.0d0
    B_vec(1) = 0.0d0
    B_vec(2) = -0.5d0 * gkl(j, cj) * (sigma(sz) + photonb_ss)
    ...
    B_vec = 0.0d0
    B_vec(1) = 0.0d0
    B_vec(2) = -0.5d0 * gkl(j, cj) * ((sigma(sz) + 0.5d0 * k1_sigma(sz)) + photonb_ss)
    ...
    B_vec = 0.0d0
    B_vec(1) = 0.0d0
    B_vec(2) = -0.5d0 * gkl(j, cj) * ((sigma(sz) + 0.5d0 * k2_sigma(sz)) + photonb_ss)
    ...
    B_vec = 0.0d0
    B_vec(1) = 0.0d0
    B_vec(2) = -0.5d0 * gkl(j, cj) * ((sigma(sz) + k3_sigma(sz)) + photonb_ss)
    ...
    ! Set the non-homogeneous vector
    B_vec = 0.0d0
    B_vec(1) = -0.5d0 * CONJG(gkl(j, cj)) * (sigma(sz) + photonb_ss)
    B_vec(2) = 0.0d0
    ...
    ! Set the non-homogeneous vector
    B_vec = 0.0d0
    B_vec(1) = -0.5d0 * CONJG(gkl(j, cj)) * ((sigma(sz) + 0.5d0 * k1_sigma(sz)) + photonb_ss)
    B_vec(2) = 0.0d0
    ...
    ! Set the non-homogeneous vector
    B_vec = 0.0d0
    B_vec(1) = -0.5d0 * CONJG(gkl(j, cj)) * ((sigma(sz) + 0.5d0 * k2_sigma(sz)) + photonb_ss)
    B_vec(2) = 0.0d0
    ...
    ! Set the non-homogeneous vector
    B_vec = 0.0d0
    B_vec(1) = -0.5d0 * CONJG(gkl(j, cj)) * ((sigma(sz) + k3_sigma(sz)) + photonb_ss)
    B_vec(2) = 0.0d0

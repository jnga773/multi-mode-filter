...
COMPLEX(KIND=8), DIMENSION(:, :, :, :), ALLOCATABLE    :: cav3a_ss, cav3b_ss
! Fourth-order moments: Cavity and atom ( < a^{\dagger} a^{2} \sigma >, < a^{\dagger 2} a \sigma >)
COMPLEX(KIND=8), DIMENSION(:, :, :, :, :), ALLOCATABLE :: cavsig4a_ss, cavsig4b_ss
...
! Third-order: Cavity
ALLOCATE(cav3a_ss(-N:N, -N:N, -N:N, 2)); cav3a_ss = 0.0d0
ALLOCATE(cav3b_ss(-N:N, -N:N, -N:N, 2)); cav3b_ss = 0.0d0

! Fourth-order: Cavity and atom
ALLOCATE(cavsig4a_ss(-N:N, -N:N, -N:N, 2, N_mat)); cavsig4a_ss = 0.0d0
ALLOCATE(cavsig4b_ss(-N:N, -N:N, -N:N, 2, N_mat)); cavsig4b_ss = 0.0d0
...
DO j = -N, N
  ...
  ! < a_{j} > !
  !-----------!
  cav1a_ss(j, a) = -gkl(j, 1) * sigma_ss(sm) / &
                 & (kappa(1) + i * wl(j, 1))
  !-----------!
  ! < b_{j} > !
  !-----------!
  cav1b_ss(j, a) = -gkl(j, 2) * sigma_ss(sm) / &
                 & (kappa(2) + i * wl(j, 2))
  !---------------------!
  ! < a^{\dagger}_{j} > !
  !---------------------!
  cav1a_ss(j, at) = -CONJG(gkl(j, 1)) * sigma_ss(sp) / &
                  & (kappa(1) - i * wl(j, 1))
  !---------------------!
  ! < b^{\dagger}_{j} > !
  !---------------------!
  cav1b_ss(j, at) = -CONJG(gkl(j, 2)) * sigma_ss(sp) / &
                  & (kappa(2) - i * wl(j, 2))
  ...
  Mat = Mat_OG
  DO x = 1, N_mat
    Mat(x, x) = Mat(x, x) - (kappa(1) + i * wl(j, 1))
  END DO

  ! Set the non-homogeneous vector
  B = 0.0d0
  B(1) = 0.0d0
  B(2) = -0.5d0 * gkl(j, 1) * (sigma_ss(sz) + 1.0d0)
  B(3) = -gamma * cav1a_ss(j, a) + &
       & gkl(j, 1) * sigma_ss(sm)
  ...
  cavsig2a_ss(j, a, :) = -MATMUL(Mat_inv, B)
  ...
  Mat = Mat_OG
  DO x = 1, N_mat
    Mat(x, x) = Mat(x, x) - (kappa(2) + i * wl(j, 2))
  END DO

  ! Set the non-homogeneous vector
  B = 0.0d0
  B(1) = 0.0d0
  B(2) = -0.5d0 * gkl(j, 2) * (sigma_ss(sz) + 1.0d0)
  B(3) = -gamma * cav1b_ss(j, a) + &
       & gkl(j, 2) * sigma_ss(sm)
  ...
  ! Calculate steady state
  cavsig2b_ss(j, a, :) = -MATMUL(Mat_inv, B)
  ...
  ! Set the non-homogeneous vector
  B = 0.0d0
  B(1) = -0.5d0 * CONJG(gkl(j, 1)) * (sigma_ss(sz) + 1.0d0)
  B(2) = 0.0d0
  B(3) = -gamma * cav1a_ss(j, at) + &
       & CONJG(gkl(j, 1)) * sigma_ss(sp)
  ...
  ! Calculate steady state
  cavsig2a_ss(j, at, :) = -MATMUL(Mat_inv, B)
  ...
  ! Set the non-homogeneous vector
  B = 0.0d0
  B(1) = -0.5d0 * CONJG(gkl(j, 2)) * (sigma_ss(sz) + 1.0d0)
  B(2) = 0.0d0
  B(3) = -gamma * cav1b_ss(j, at) + &
       & CONJG(gkl(j, 2)) * sigma_ss(sp)
  ...
  ! Calculate steady state
  cavsig2b_ss(j, at, :) = -MATMUL(Mat_inv, B)
  ...
  DO cj = 1, 2
    ...
    DO k = -N, N
      DO j = -N, N
        ...
        ! < a_{j} a_{k} > !
        !-----------------!
        cav2_ss(j, k, a, cj, ck) = -gkl(j, cj) * cavsig2a_ss(k, a, sm) + &
                                 & -gkl(k, ck) * cavsig2a_ss(j, a, sm)
        ...
        !-------------------------------------!
        cav2_ss(j, k, at, cj, ck) = -CONJG(gkl(j, cj)) * cavsig2a_ss(k, at, sp) + &
                                  & -CONJG(gkl(k, ck)) * cavsig2a_ss(j, at, sp)
        ...
        ! < a^{\dagger}_{j} a_{k} > !
        !---------------------------!
        cav2_ss(j, k, ata, cj, ck) = -CONJG(gkl(j, cj)) * cavsig2a_ss(k, a, sp) + &
                                   & -gkl(k, ck) * cavsig2a_ss(j, at, sm)
        ...
        B = 0.0d0
        B(1) = 0.0d0
        B(2) = -0.5d0 * gkl(j, cj) * cavsig2a_ss(k, a, sz) + &
             & -0.5d0 * gkl(j, cj) * cav1a_ss(k, a) + &
             & -0.5d0 * gkl(k, ck) * cavsig2a_ss(j, a, sz) + &
             & -0.5d0 * gkl(k, ck) * cav1a_ss(j, a)
        B(3) = -gamma * cav2_ss(j, k, a, cj, ck) + &
             & gkl(j, cj) * cavsig2a_ss(k, a, sm) + &
             & gkl(k, ck) * cavsig2a_ss(j, a, sm)
        ...
        ! Set the non-homogeneous vector
        B = 0.0d0
        B(1) = -0.5d0 * CONJG(gkl(j, cj)) * cavsig2a_ss(k, at, sz) + &
             & -0.5d0 * CONJG(gkl(j, cj)) * cav1a_ss(k, at) + &
             & -0.5d0 * CONJG(gkl(k, ck)) * cavsig2a_ss(j, at, sz) + &
             & -0.5d0 * CONJG(gkl(k, ck)) * cav1a_ss(j, at)
        B(2) = 0.0d0
        B(3) = -gamma * cav2_ss(j, k, at, cj, ck) + &
             & CONJG(gkl(j, cj)) * cavsig2a_ss(k, at, sp) + &
             & CONJG(gkl(k, ck)) * cavsig2a_ss(j, at, sp)
        ...
        ! Set the non-homogeneous vector
        B = 0.0d0
        B(1) = -0.5d0 * CONJG(gkl(j, cj)) * cavsig2a_ss(k, a, sz) + &
             & -0.5d0 * CONJG(gkl(j, cj)) * cav1a_ss(k, a)
        B(2) = -0.5d0 * gkl(k, ck) * cavsig2a_ss(j, at, sz) + &
             & -0.5d0 * gkl(k, ck) * cav1a_ss(j, at)
        B(3) = -gamma * cav2_ss(j, k, ata, cj, ck) + &
             & CONJG(gkl(j, cj)) * cavsig2a_ss(k, a, sp) + &
             & gkl(k, ck) * cavsig2a_ss(j, at, sm)
...
DO l = -N, N
  DO k = -N, N
    DO j = -N, N
      ...
      !---------------------------------!
      cav3a_ss(j, k, l, a) = -CONJG(gkl(j, 1)) * cavsig3_ss(k, l, a, sp, 1, 2) + &
                           & -gkl(k, 1) * cavsig3_ss(j, l, ata, sm, 1, 2) + &
                           & -gkl(l, 2) * cavsig3_ss(j, k, ata, sm, 1, 1)
      cav3a_ss(j, k, l, a) = cav3a_ss(j, k, l, a) / &
                           & ((2.0d0 * kappa(1)) + kappa(2) - i * (wl(j, 1) - wl(k, 1) - wl(l, 2)))

      !---------------------------------!
      ! < b^{\dagger}_{j} b_{k} a_{l} > !
      !---------------------------------!
      cav3b_ss(j, k, l, a) = -CONJG(gkl(j, 2)) * cavsig3_ss(k, l, a, sp, 2, 1) + &
                           & -gkl(k, 2) * cavsig3_ss(j, l, ata, sm, 2, 1) + &
                           & -gkl(l, 1) * cavsig3_ss(j, k, ata, sm, 2, 2)
      cav3b_ss(j, k, l, a) = cav3b_ss(j, k, l, a) / &
                           & (kappa(1) + (2.0d0 * kappa(2)) - i * (wl(j, 2) - wl(k, 2) - wl(l, 1)))

      !-------------------------------------------!
      ! < b^{\dagger}_{j} a^{\dagger}_{k} a_{l} > !
      !-------------------------------------------!
      cav3a_ss(j, k, l, at) = -CONJG(gkl(j, 2)) * cavsig3_ss(k, l, ata, sp, 1, 1) + &
                            & -CONJG(gkl(k, 1)) * cavsig3_ss(j, l, ata, sp, 2, 1) + &
                            & -gkl(l, 1) * cavsig3_ss(j, k, at, sm, 2, 1)
      cav3a_ss(j, k, l, at) = cav3a_ss(j, k, l, at) / &
                            & ((2.0d0 * kappa(1)) + kappa(2) - i * (wl(j, 2) + wl(k, 1) - wl(l, 1)))

      !-------------------------------------------!
      ! < a^{\dagger}_{j} b^{\dagger}_{k} b_{l} > !
      !-------------------------------------------!
      cav3b_ss(j, k, l, at) = -CONJG(gkl(j, 1)) * cavsig3_ss(k, l, ata, sp, 2, 2) + &
                            & -CONJG(gkl(k, 2)) * cavsig3_ss(j, l, ata, sp, 1, 2) + &
                            & -gkl(l, 2) * cavsig3_ss(j, k, at, sm, 1, 2)
      cav3b_ss(j, k, l, at) = cav3b_ss(j, k, l, at) / &
                            & (kappa(1) + (2.0d0 * kappa(2)) - i * (wl(j, 1) + wl(k, 2) - wl(l, 2)))
      ...
      Mat = Mat_OG
      DO x = 1, N_mat
        Mat(x, x) = Mat(x, x) - ((2.0d0 * kappa(1)) + kappa(2) - i * (wl(j, 1) - wl(k, 1) - wl(l, 2)))
      END DO

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -0.5d0 * CONJG(gkl(j, 1)) * cavsig3_ss(k, l, a, sz, 1, 2) + &
           & -0.5d0 * CONJG(gkl(j, 1)) * cav2_ss(k, l, a, 1, 2)
      B(2) = -0.5d0 * gkl(k, 1) * cavsig3_ss(j, l, ata, sz, 1, 2) + &
           & -0.5d0 * gkl(k, 1) * cav2_ss(j, l, ata, 1, 2) + &
           & -0.5d0 * gkl(l, 2) * cavsig3_ss(j, k, ata, sz, 1, 1) + &
           & -0.5d0 * gkl(l, 2) * cav2_ss(j, k, ata, 1, 1)
      B(3) = -gamma * cav3a_ss(j, k, l, a) + &
           & CONJG(gkl(j, 1)) * cavsig3_ss(k, l, a, sp, 1, 2) + &
           & gkl(k, 1) * cavsig3_ss(j, l, ata, sm, 1, 2) + &
           & gkl(l, 2) * cavsig3_ss(j, k, ata, sm, 1, 1)
      ...
      ! Calculate steady state
      cavsig4a_ss(j, k, l, a, :) = -MATMUL(Mat_inv, B)
      ...
      Mat = Mat_OG
      DO x = 1, N_mat
        Mat(x, x) = Mat(x, x) - (kappa(1) + (2.0d0 * kappa(2)) - i * (wl(j, 2) - wl(k, 2) - wl(l, 1)))
      END DO

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -0.5d0 * CONJG(gkl(j, 2)) * cavsig3_ss(k, l, a, sz, 2, 1) + &
           & -0.5d0 * CONJG(gkl(j, 2)) * cav2_ss(k, l, a, 2, 1)
      B(2) = -0.5d0 * gkl(k, 2) * cavsig3_ss(j, l, ata, sz, 2, 1) + &
           & -0.5d0 * gkl(k, 2) * cav2_ss(j, l, ata, 2, 1) + &
           & -0.5d0 * gkl(l, 1) * cavsig3_ss(j, k, ata, sz, 2, 2) + &
           & -0.5d0 * gkl(l, 1) * cav2_ss(j, k, ata, 2, 2)
      B(3) = -gamma * cav3b_ss(j, k, l, a) + &
           & CONJG(gkl(j, 2)) * cavsig3_ss(k, l, a, sp, 2, 1) + &
           & gkl(k, 2) * cavsig3_ss(j, l, ata, sm, 2, 1) + &
           & gkl(l, 1) * cavsig3_ss(j, k, ata, sm, 2, 2)
      ...
      ! Calculate steady state
      cavsig4b_ss(j, k, l, a, :) = -MATMUL(Mat_inv, B)
      ...
      Mat = Mat_OG
      DO x = 1, N_mat
        Mat(x, x) = Mat(x, x) - ((2.0d0 * kappa(1)) + kappa(2) - i * (wl(j, 2) + wl(k, 1) - wl(l, 1)))
      END DO

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -0.5d0 * CONJG(gkl(j, 2)) * cavsig3_ss(k, l, ata, sz, 1, 1) + &
           & -0.5d0 * CONJG(gkl(j, 2)) * cav2_ss(k, l, ata, 1, 1) + &
           & -0.5d0 * CONJG(gkl(k, 1)) * cavsig3_ss(j, l, ata, sz, 2, 1) + &
           & -0.5d0 * CONJG(gkl(k, 1)) * cav2_ss(j, l, ata, 2, 1)
      B(2) = -0.5d0 * gkl(l, 1) * cavsig3_ss(j, k, at, sz, 2, 1) + &
           & -0.5d0 * gkl(l, 1) * cav2_ss(j, k, at, 2, 1)
      B(3) = -gamma * cav3a_ss(j, k, l, at) + &
           & CONJG(gkl(j, 2)) * cavsig3_ss(k, l, ata, sp, 1, 1) + &
           & CONJG(gkl(k, 1)) * cavsig3_ss(j, l, ata, sp, 2, 1) + &
           & gkl(l, 1) * cavsig3_ss(j, k, at, sm, 2, 1)
      ...
      ! Calculate steady state
      cavsig4a_ss(j, k, l, at, :) = -MATMUL(Mat_inv, B)
      ...
      Mat = Mat_OG
      DO x = 1, N_mat
        Mat(x, x) = Mat(x, x) - (kappa(1) + (2.0d0 * kappa(2)) - i * (wl(j, 1) + wl(k, 2) - wl(l, 2)))
      END DO

      ! Set the non-homogeneous vector
      B = 0.0d0
      B(1) = -0.5d0 * CONJG(gkl(j, 1)) * cavsig3_ss(k, l, ata, sz, 2, 2) + &
           & -0.5d0 * CONJG(gkl(j, 1)) * cav2_ss(k, l, ata, 2, 2) + &
           & -0.5d0 * CONJG(gkl(k, 2)) * cavsig3_ss(j, l, ata, sz, 1, 2) + &
           & -0.5d0 * CONJG(gkl(k, 2)) * cav2_ss(j, l, ata, 1, 2)
      B(2) = -0.5d0 * gkl(l, 2) * cavsig3_ss(j, k, at, sz, 1, 2) + &
           & -0.5d0 * gkl(l, 2) * cav2_ss(j, k, at, 1, 2)
      B(3) = -gamma * cav3b_ss(j, k, l, at) + &
           & CONJG(gkl(j, 1)) * cavsig3_ss(k, l, ata, sp, 2, 2) + &
           & CONJG(gkl(k, 2)) * cavsig3_ss(j, l, ata, sp, 1, 2) + &
           & gkl(l, 2) * cavsig3_ss(j, k, at, sm, 1, 2)
      ...
      ! Calculate steady state
      cavsig4b_ss(j, k, l, at, :) = -MATMUL(Mat_inv, B)
  ...
  DO l = -N, N
    DO k = -N, N
      DO j = -N, N
        ...
        !-------------------------------------------------!
        cav4_ss(j, k, l, m) = -CONJG(gkl(j, 1)) * cavsig4b_ss(k, l, m, a, sp) + &
                            & -CONJG(gkl(k, 2)) * cavsig4a_ss(j, m, l, a, sp) + &
                            & -gkl(l, 2) * cavsig4a_ss(k, j, m, at, sm) + &
                            & -gkl(m, 1) * cavsig4b_ss(j, k, l, at, sm)
...
DO m = -N, N
  DO j = -N, N
    ...
    DO k = -N, N
      ! First-order: Cavity
      cav1(k, a) = cav1(k, a) + cav3a_ss(j, m, k, a)
      cav1(k, at) = cav1(k, at) + cav3a_ss(k, j, m, at)
      ! Second-order: Cavity and atom
      cavsig2(k, a, :) = cavsig2(k, a, :) + cavsig4a_ss(j, m, k, a, :)
      cavsig2(k, at, :) = cavsig2(k, at, :) + cavsig4a_ss(k, j, m, at, :)

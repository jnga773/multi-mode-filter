...
DO t = 0, tau_steps
  ...
  DO k = -N, N
    ...
    B(1) = 0.0d0
    B(2) = 0.0d0
    B(3) = -gamma * cav1_ss(k, a)

    ! Calculate k1
    k1_cavsig2(k, a, :) = dt * (MATMUL(Mat, cavsig2(k, a, :)) + B)
    k2_cavsig2(k, a, :) = dt * (MATMUL(Mat, (cavsig2(k, a, :) + 0.5d0 * k1_cavsig2(k, a, :))) + B)
    k3_cavsig2(k, a, :) = dt * (MATMUL(Mat, (cavsig2(k, a, :) + 0.5d0 * k2_cavsig2(k, a, :))) + B)
    k4_cavsig2(k, a, :) = dt * (MATMUL(Mat, (cavsig2(k, a, :) + k3_cavsig2(k, a, :))) + B)

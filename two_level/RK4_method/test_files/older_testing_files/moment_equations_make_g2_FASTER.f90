...
IF (tau2_max > 0.0) THEN
  ...
  DO t = 0, tau_steps
    ...
    ! k1 - First-order: Bloch equations (<sm>, <sp>, <sz>)
    k1_cavsig3(:, :, ata, sm) = -dt * 0.5d0 * gamma * cavsig3(:, :, ata, sm) + &
                              & dt * i * 0.5d0 * Omega * cavsig3(:, :, ata, sz)
    k1_cavsig3(:, :, ata, sp) = -dt * 0.5d0 * gamma * cavsig3(:, :, ata, sp) - &
                              & dt * i * 0.5d0 * Omega * cavsig3(:, :, ata, sz)
    k1_cavsig3(:, :, ata, sz) = -dt * gamma * cavsig3(:, :, ata, sz) + &
                              & dt * i * Omega * cavsig3(:, :, ata, sm) - &
                              & dt * i * Omega * cavsig3(:, :, ata, sp) - &
                              & dt * gamma * cav2_ss(:, :, ata)

    ! k2 - First-order: Bloch equations (<sm>, <sp>, <sz>)
    k2_cavsig3(:, :, ata, sm) = -dt * 0.5d0 * gamma * (cavsig3(:, :, ata, sm) + 0.5d0 * k1_cavsig3(:, :, ata, sm)) + &
                              & dt * i * 0.5d0 * Omega * (cavsig3(:, :, ata, sz) + 0.5d0 * k1_cavsig3(:, :, ata, sz))
    k2_cavsig3(:, :, ata, sp) = -dt * 0.5d0 * gamma * (cavsig3(:, :, ata, sp) + 0.5d0 * k1_cavsig3(:, :, ata, sp)) - &
                              & dt * i * 0.5d0 * Omega * (cavsig3(:, :, ata, sz) + 0.5d0 * k1_cavsig3(:, :, ata, sz))
    k2_cavsig3(:, :, ata, sz) = -dt * gamma * (cavsig3(:, :, ata, sz) + 0.5d0 * k1_cavsig3(:, :, ata, sz)) + &
                              & dt * i * Omega * (cavsig3(:, :, ata, sm) + 0.5d0 * k1_cavsig3(:, :, ata, sm)) - &
                              & dt * i * Omega * (cavsig3(:, :, ata, sp) + 0.5d0 * k1_cavsig3(:, :, ata, sp)) - &
                              & dt * gamma * cav2_ss(:, :, ata)

    ! k3 - First-order: Bloch equations (<sm>, <sp>, <sz>)
    k3_cavsig3(:, :, ata, sm) = -dt * 0.5d0 * gamma * (cavsig3(:, :, ata, sm) + 0.5d0 * k2_cavsig3(:, :, ata, sm)) + &
                              & dt * i * 0.5d0 * Omega * (cavsig3(:, :, ata, sz) + 0.5d0 * k2_cavsig3(:, :, ata, sz))
    k3_cavsig3(:, :, ata, sp) = -dt * 0.5d0 * gamma * (cavsig3(:, :, ata, sp) + 0.5d0 * k2_cavsig3(:, :, ata, sp)) - &
                              & dt * i * 0.5d0 * Omega * (cavsig3(:, :, ata, sz) + 0.5d0 * k2_cavsig3(:, :, ata, sz))
    k3_cavsig3(:, :, ata, sz) = -dt * gamma * (cavsig3(:, :, ata, sz) + 0.5d0 * k2_cavsig3(:, :, ata, sz)) + &
                              & dt * i * Omega * (cavsig3(:, :, ata, sm) + 0.5d0 * k2_cavsig3(:, :, ata, sm)) - &
                              & dt * i * Omega * (cavsig3(:, :, ata, sp) + 0.5d0 * k2_cavsig3(:, :, ata, sp)) - &
                              & dt * gamma * cav2_ss(:, :, ata)

    ! k4 - First-order: Bloch equations (<sm>, <sp>, <sz>)
    k4_cavsig3(:, :, ata, sm) = -dt * 0.5d0 * gamma * (cavsig3(:, :, ata, sm) + k3_cavsig3(:, :, ata, sm)) + &
                              & dt * i * 0.5d0 * Omega * (cavsig3(:, :, ata, sz) + k3_cavsig3(:, :, ata, sz))
    k4_cavsig3(:, :, ata, sp) = -dt * 0.5d0 * gamma * (cavsig3(:, :, ata, sp) + k3_cavsig3(:, :, ata, sp)) - &
                              & dt * i * 0.5d0 * Omega * (cavsig3(:, :, ata, sz) + k3_cavsig3(:, :, ata, sz))
    k4_cavsig3(:, :, ata, sz) = -dt * gamma * (cavsig3(:, :, ata, sz) + k3_cavsig3(:, :, ata, sz)) + &
                              & dt * i * Omega * (cavsig3(:, :, ata, sm) + k3_cavsig3(:, :, ata, sm)) - &
                              & dt * i * Omega * (cavsig3(:, :, ata, sp) + k3_cavsig3(:, :, ata, sp)) - &
                              & dt * gamma * cav2_ss(:, :, ata)
    ...
    DO l = -N, N
      ...
      !-----------------------------!
      ! k1 - First-order: Cavity mode (<a>, <at>)
      k1_cav3(:, l, :, a) = -dt * (kappa + i * wl(l)) * cav3(:, l, :, a) - &
                          & dt * gkal(l) * cavsig3(:, :, ata, sm)

      ! k2 - First-order: Cavity mode (<a>, <at>)
      k2_cav3(:, l, :, a) = -dt * (kappa + i * wl(l)) * (cav3(:, l, :, a) + 0.5d0 * k1_cav3(:, l, :, a)) - &
                          & dt * gkal(l) * (cavsig3(:, :, ata, sm) + 0.5d0 * k1_cavsig3(:, :, ata, sm))

      ! k3 - First-order: Cavity mode (<a>, <at>)
      k3_cav3(:, l, :, a) = -dt * (kappa + i * wl(l)) * (cav3(:, l, :, a) + 0.5d0 * k2_cav3(:, l, :, a)) - &
                          & dt * gkal(l) * (cavsig3(:, :, ata, sm) + 0.5d0 * k2_cavsig3(:, :, ata, sm))

      ! k4 - First-order: Cavity mode (<a>, <at>)
      k4_cav3(:, l, :, a) = -dt * (kappa + i * wl(l)) * (cav3(:, l, :, a) + k3_cav3(:, l, :, a)) - &
                          & dt * gkal(l) * (cavsig3(:, :, ata, sm) + k3_cavsig3(:, :, ata, sm))

      !---------------------------------------!
      !     Second-Order: Cavity and Atom     !
      !---------------------------------------!
      ! k1 - Second-order: a and \sigma (<a sm>, <a sp>, <a sz>)
      k1_cavsig4(:, l, :, a, sm) = -dt * (0.5d0 * gamma + kappa + i * wl(l)) * cavsig4(:, l, :, a, sm) + &
                                 & dt * i * 0.5d0 * Omega * cavsig4(:, l, :, a, sz)
      k1_cavsig4(:, l, :, a, sp) = -dt * (0.5d0 * gamma + kappa + i * wl(l)) * cavsig4(:, l, :, a, sp) - &
                                 & dt * i * 0.5d0 * Omega * cavsig4(:, l, :, a, sz) - &
                                 & dt * 0.5d0 * gkal(l) * (cavsig3(:, :, ata, sz) + cav2_ss(:, :, ata))
      k1_cavsig4(:, l, :, a, sz) = -dt * (gamma + kappa + i * wl(l)) * cavsig4(:, l, :, a, sz) + &
                                 & dt * i * Omega * cavsig4(:, l, :, a, sm) - &
                                 & dt * i * Omega * cavsig4(:, l, :, a, sp) - &
                                 & dt * gamma * cav3(:, l, :, a) + &
                                 & dt * gkal(l) * cavsig3(:, :, ata, sm)

      ! k2 - Second-order: a and \sigma (<a sm>, <a sp>, <a sz>)
      k2_cavsig4(:, l, :, a, sm) = -dt * (0.5d0 * gamma + kappa + i * wl(l)) * (cavsig4(:, l, :, a, sm) + 0.5d0 * k1_cavsig4(:, l, :, a, sm)) + &
                                 & dt * i * 0.5d0 * Omega * (cavsig4(:, l, :, a, sz) + 0.5d0 * k1_cavsig4(:, l, :, a, sz))
      k2_cavsig4(:, l, :, a, sp) = -dt * (0.5d0 * gamma + kappa + i * wl(l)) * (cavsig4(:, l, :, a, sp) + 0.5d0 * k1_cavsig4(:, l, :, a, sp)) - &
                                 & dt * i * 0.5d0 * Omega * (cavsig4(:, l, :, a, sz) + 0.5d0 * k1_cavsig4(:, l, :, a, sz)) - &
                                 & dt * 0.5d0 * gkal(l) * ((cavsig3(:, :, ata, sz) + 0.5d0 * k1_cavsig3(:, :, ata, sz)) + cav2_ss(:, :, ata))
      k2_cavsig4(:, l, :, a, sz) = -dt * (gamma + kappa + i * wl(l)) * (cavsig4(:, l, :, a, sz) + 0.5d0 * k1_cavsig4(:, l, :, a, sz)) + &
                                 & dt * i * Omega * (cavsig4(:, l, :, a, sm) + 0.5d0 * k1_cavsig4(:, l, :, a, sm)) - &
                                 & dt * i * Omega * (cavsig4(:, l, :, a, sp) + 0.5d0 * k1_cavsig4(:, l, :, a, sp)) - &
                                 & dt * gamma * (cav3(:, l, :, a) + 0.5d0 * k1_cav3(:, l, :, a)) + &
                                 & dt * gkal(l) * (cavsig3(:, :, ata, sm) + 0.5d0 * k1_cavsig3(:, :, ata, sm))

      ! k3 - Second-order: a and \sigma (<a sm>, <a sp>, <a sz>)
      k3_cavsig4(:, l, :, a, sm) = -dt * (0.5d0 * gamma + kappa + i * wl(l)) * (cavsig4(:, l, :, a, sm) + 0.5d0 * k2_cavsig4(:, l, :, a, sm)) + &
                                 & dt * i * 0.5d0 * Omega * (cavsig4(:, l, :, a, sz) + 0.5d0 * k2_cavsig4(:, l, :, a, sz))
      k3_cavsig4(:, l, :, a, sp) = -dt * (0.5d0 * gamma + kappa + i * wl(l)) * (cavsig4(:, l, :, a, sp) + 0.5d0 * k2_cavsig4(:, l, :, a, sp)) - &
                                 & dt * i * 0.5d0 * Omega * (cavsig4(:, l, :, a, sz) + 0.5d0 * k2_cavsig4(:, l, :, a, sz)) - &
                                 & dt * 0.5d0 * gkal(l) * ((cavsig3(:, :, ata, sz) + 0.5d0 * k2_cavsig3(:, :, ata, sz)) + cav2_ss(:, :, ata))
      k3_cavsig4(:, l, :, a, sz) = -dt * (gamma + kappa + i * wl(l)) * (cavsig4(:, l, :, a, sz) + 0.5d0 * k2_cavsig4(:, l, :, a, sz)) + &
                                 & dt * i * Omega * (cavsig4(:, l, :, a, sm) + 0.5d0 * k2_cavsig4(:, l, :, a, sm)) - &
                                 & dt * i * Omega * (cavsig4(:, l, :, a, sp) + 0.5d0 * k2_cavsig4(:, l, :, a, sp)) - &
                                 & dt * gamma * (cav3(:, l, :, a) + 0.5d0 * k2_cav3(:, l, :, a)) + &
                                 & dt * gkal(l) * (cavsig3(:, :, ata, sm) + 0.5d0 * k2_cavsig3(:, :, ata, sm))

      ! k4 - Second-order: a and \sigma (<a sm>, <a sp>, <a sz>)
      k4_cavsig4(:, l, :, a, sm) = -dt * (0.5d0 * gamma + kappa + i * wl(l)) * (cavsig4(:, l, :, a, sm) + k3_cavsig4(:, l, :, a, sm)) + &
                                 & dt * i * 0.5d0 * Omega * (cavsig4(:, l, :, a, sz) + k3_cavsig4(:, l, :, a, sz))
      k4_cavsig4(:, l, :, a, sp) = -dt * (0.5d0 * gamma + kappa + i * wl(l)) * (cavsig4(:, l, :, a, sp) + k3_cavsig4(:, l, :, a, sp)) - &
                                 & dt * i * 0.5d0 * Omega * (cavsig4(:, l, :, a, sz) + k3_cavsig4(:, l, :, a, sz)) - &
                                 & dt * 0.5d0 * gkal(l) * ((cavsig3(:, :, ata, sz) + k3_cavsig3(:, :, ata, sz)) + cav2_ss(:, :, ata))
      k4_cavsig4(:, l, :, a, sz) = -dt * (gamma + kappa + i * wl(l)) * (cavsig4(:, l, :, a, sz) + k3_cavsig4(:, l, :, a, sz)) + &
                                 & dt * i * Omega * (cavsig4(:, l, :, a, sm) + k3_cavsig4(:, l, :, a, sm)) - &
                                 & dt * i * Omega * (cavsig4(:, l, :, a, sp) + k3_cavsig4(:, l, :, a, sp)) - &
                                 & dt * gamma * (cav3(:, l, :, a) + k3_cav3(:, l, :, a)) + &
                                 & dt * gkal(l) * (cavsig3(:, :, ata, sm) + k3_cavsig3(:, :, ata, sm))
      ...
      DO k = -N, N
        ...
        !-----------------------------!
        ! k1 - First-order: Cavity mode <at>
        k1_cav3(:, k, :, at) = -dt * (kappa - i * wl(k)) * cav3(:, k, :, at) - &
                             & dt * CONJG(gkal(k)) * cavsig3(:, :, ata, sp)

        ! k2 - First-order: Cavity mode <at>
        k2_cav3(:, k, :, at) = -dt * (kappa - i * wl(k)) * (cav3(:, k, :, at) + 0.5d0 * k1_cav3(:, k, :, at)) - &
                             & dt * CONJG(gkal(k)) * (cavsig3(:, :, ata, sp) + 0.5d0 * k1_cavsig3(:, :, ata, sp))

        ! k3 - First-order: Cavity mode <at>
        k3_cav3(:, k, :, at) = -dt * (kappa - i * wl(k)) * (cav3(:, k, :, at) + 0.5d0 * k2_cav3(:, k, :, at)) - &
                             & dt * CONJG(gkal(k)) * (cavsig3(:, :, ata, sp) + 0.5d0 * k2_cavsig3(:, :, ata, sp))

        ! k4 - First-order: Cavity mode <at>
        k4_cav3(:, k, :, at) = -dt * (kappa - i * wl(k)) * (cav3(:, k, :, at) + k3_cav3(:, k, :, at)) - &
                             & dt * CONJG(gkal(k)) * (cavsig3(:, :, ata, sp) + k3_cavsig3(:, :, ata, sp))

        !---------------------------------------!
        !     Second-Order: Cavity and Atom     !
        !---------------------------------------!
        ! k1 - Second-order: at and \sigma (<at sm>, <at sp>, <at sz>)
        k1_cavsig4(:, k, :, at, sm) = -dt * (0.5d0 * gamma + kappa - i * wl(k)) * cavsig4(:, k, :, at, sm) + &
                                    & dt * i * 0.5d0 * Omega * cavsig4(:, k, :, at, sz) - &
                                    & dt * 0.5d0 * CONJG(gkal(k)) * (cavsig3(:, :, ata, sz) + cav2_ss(:, :, ata))
        k1_cavsig4(:, k, :, at, sp) = -dt * (0.5d0 * gamma + kappa - i * wl(k)) * cavsig4(:, k, :, at, sp) - &
                                    & dt * i * 0.5d0 * Omega * cavsig4(:, k, :, at, sz)
        k1_cavsig4(:, k, :, at, sz) = -dt * (gamma + kappa - i * wl(k)) * cavsig4(:, k, :, at, sz) + &
                                    & dt * i * Omega * cavsig4(:, k, :, at, sm) - &
                                    & dt * i * Omega * cavsig4(:, k, :, at, sp) - &
                                    & dt * gamma * cav3(:, k, :, at) + &
                                    & dt * CONJG(gkal(k)) * cavsig3(:, :, ata, sp)

        ! k2 - Second-order: at and \sigma (<at sm>, <at sp>, <at sz>)
        k2_cavsig4(:, k, :, at, sm) = -dt * (0.5d0 * gamma + kappa - i * wl(k)) * (cavsig4(:, k, :, at, sm) + 0.5d0 * k1_cavsig4(:, k, :, at, sm)) + &
                                    & dt * i * 0.5d0 * Omega * (cavsig4(:, k, :, at, sz) + 0.5d0 * k1_cavsig4(:, k, :, at, sz)) - &
                                    & dt * 0.5d0 * CONJG(gkal(k)) * ((cavsig3(:, :, ata, sz) + 0.5d0 * k1_cavsig3(:, :, ata, sz)) + cav2_ss(:, :, ata))
        k2_cavsig4(:, k, :, at, sp) = -dt * (0.5d0 * gamma + kappa - i * wl(k)) * (cavsig4(:, k, :, at, sp) + 0.5d0 * k1_cavsig4(:, k, :, at, sp)) - &
                                    & dt * i * 0.5d0 * Omega * (cavsig4(:, k, :, at, sz) + 0.5d0 * k1_cavsig4(:, k, :, at, sz))
        k2_cavsig4(:, k, :, at, sz) = -dt * (gamma + kappa - i * wl(k)) * (cavsig4(:, k, :, at, sz) + 0.5d0 * k1_cavsig4(:, k, :, at, sz)) + &
                                    & dt * i * Omega * (cavsig4(:, k, :, at, sm) + 0.5d0 * k1_cavsig4(:, k, :, at, sm)) - &
                                    & dt * i * Omega * (cavsig4(:, k, :, at, sp) + 0.5d0 * k1_cavsig4(:, k, :, at, sp)) - &
                                    & dt * gamma * (cav3(:, k, :, at) + 0.5d0 * k1_cav3(:, k, :, at)) + &
                                    & dt * CONJG(gkal(k)) * (cavsig3(:, :, ata, sp) + 0.5d0 * k1_cavsig3(:, :, ata, sp))

        ! k3 - Second-order: at and \sigma (<at sm>, <at sp>, <at sz>)
        k3_cavsig4(:, k, :, at, sm) = -dt * (0.5d0 * gamma + kappa - i * wl(k)) * (cavsig4(:, k, :, at, sm) + 0.5d0 * k2_cavsig4(:, k, :, at, sm)) + &
                                    & dt * i * 0.5d0 * Omega * (cavsig4(:, k, :, at, sz) + 0.5d0 * k2_cavsig4(:, k, :, at, sz)) - &
                                    & dt * 0.5d0 * CONJG(gkal(k)) * ((cavsig3(:, :, ata, sz) + 0.5d0 * k2_cavsig3(:, :, ata, sz)) + cav2_ss(:, :, ata))
        k3_cavsig4(:, k, :, at, sp) = -dt * (0.5d0 * gamma + kappa - i * wl(k)) * (cavsig4(:, k, :, at, sp) + 0.5d0 * k2_cavsig4(:, k, :, at, sp)) - &
                                    & dt * i * 0.5d0 * Omega * (cavsig4(:, k, :, at, sz) + 0.5d0 * k2_cavsig4(:, k, :, at, sz))
        k3_cavsig4(:, k, :, at, sz) = -dt * (gamma + kappa - i * wl(k)) * (cavsig4(:, k, :, at, sz) + 0.5d0 * k2_cavsig4(:, k, :, at, sz)) + &
                                    & dt * i * Omega * (cavsig4(:, k, :, at, sm) + 0.5d0 * k2_cavsig4(:, k, :, at, sm)) - &
                                    & dt * i * Omega * (cavsig4(:, k, :, at, sp) + 0.5d0 * k2_cavsig4(:, k, :, at, sp)) - &
                                    & dt * gamma * (cav3(:, k, :, at) + 0.5d0 * k2_cav3(:, k, :, at)) + &
                                    & dt * CONJG(gkal(k)) * (cavsig3(:, :, ata, sp) + 0.5d0 * k2_cavsig3(:, :, ata, sp))

        ! k4 - Second-order: at and \sigma (<at sm>, <at sp>, <at sz>)
        k4_cavsig4(:, k, :, at, sm) = -dt * (0.5d0 * gamma + kappa - i * wl(k)) * (cavsig4(:, k, :, at, sm) + k3_cavsig4(:, k, :, at, sm)) + &
                                    & dt * i * 0.5d0 * Omega * (cavsig4(:, k, :, at, sz) + k3_cavsig4(:, k, :, at, sz)) - &
                                    & dt * 0.5d0 * CONJG(gkal(k)) * ((cavsig3(:, :, ata, sz) + k3_cavsig3(:, :, ata, sz)) + cav2_ss(:, :, ata))
        k4_cavsig4(:, k, :, at, sp) = -dt * (0.5d0 * gamma + kappa - i * wl(k)) * (cavsig4(:, k, :, at, sp) + k3_cavsig4(:, k, :, at, sp)) - &
                                    & dt * i * 0.5d0 * Omega * (cavsig4(:, k, :, at, sz) + k3_cavsig4(:, k, :, at, sz))
        k4_cavsig4(:, k, :, at, sz) = -dt * (gamma + kappa - i * wl(k)) * (cavsig4(:, k, :, at, sz) + k3_cavsig4(:, k, :, at, sz)) + &
                                    & dt * i * Omega * (cavsig4(:, k, :, at, sm) + k3_cavsig4(:, k, :, at, sm)) - &
                                    & dt * i * Omega * (cavsig4(:, k, :, at, sp) + k3_cavsig4(:, k, :, at, sp)) - &
                                    & dt * gamma * (cav3(:, k, :, at) + k3_cav3(:, k, :, at)) + &
                                    & dt * CONJG(gkal(k)) * (cavsig3(:, :, ata, sp) + k3_cavsig3(:, :, ata, sp))
        ...
        k1_cav4(:, k, l, :) = -dt * (2.0d0 * kappa - i * (wl(k) - wl(l))) * cav4(:, k, l, :) - &
                            & dt * CONJG(gkal(k)) * cavsig4(:, l, :, a, sp) - &
                            & dt * gkal(l) * cavsig4(:, k, :, at, sm)

        ! k2 - Second-order: Cavity <at a>
        k2_cav4(:, k, l, :) = -dt * (2.0d0 * kappa - i * (wl(k) - wl(l))) * (cav4(:, k, l, :) + 0.5d0 * k1_cav4(:, k, l, :)) - &
                            & dt * CONJG(gkal(k)) * (cavsig4(:, l, :, a, sp) + 0.5d0 * k1_cavsig4(:, l, :, a, sp)) - &
                            & dt * gkal(l) * (cavsig4(:, k, :, at, sm) + 0.5d0 * k1_cavsig4(:, k, :, at, sm))

        ! k3 - Second-order: Cavity <at a>
        k3_cav4(:, k, l, :) = -dt * (2.0d0 * kappa - i * (wl(k) - wl(l))) * (cav4(:, k, l, :) + 0.5d0 * k2_cav4(:, k, l, :)) - &
                            & dt * CONJG(gkal(k)) * (cavsig4(:, l, :, a, sp) + 0.5d0 * k2_cavsig4(:, l, :, a, sp)) - &
                            & dt * gkal(l) * (cavsig4(:, k, :, at, sm) + 0.5d0 * k2_cavsig4(:, k, :, at, sm))

        ! k4 - Second-order: Cavity <at a>
        k4_cav4(:, k, l, :) = -dt * (2.0d0 * kappa - i * (wl(k) - wl(l))) * (cav4(:, k, l, :) + k3_cav4(:, k, l, :)) - &
                            & dt * CONJG(gkal(k)) * (cavsig4(:, l, :, a, sp) + k3_cavsig4(:, l, :, a, sp)) - &
                            & dt * gkal(l) * (cavsig4(:, k, :, at, sm) + k3_cavsig4(:, k, :, at, sm))
      END DO
    END DO

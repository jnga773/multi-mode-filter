...
DO t = 0, t_steps
  ...
  ! Calculate
  ! First order: Bloch equations (<sm>, <sp>, <sz>)
  ...
  DO j = -N, N
    ...
    k1_cavsig2(j, a, sz) = -dt * (gamma + kappa + i * wl(j)) * cavsig2(j, a, sz) + &
                         ...
                         & dt * gamma * cav1(j, a) + &
                         & dt * gkal(j) * bloch(sm)
    ! Second-order: at and \sigma (<at sm>, <at sp>, <at sz>)
    ...
    ! Cycle through mode numbers
    DO k = -N, N
      ! Second-order: Cavity (<a a>, <at at>, <at a>)
      ...
      ! Third-order: a and \siga (<a^{2}, \sigma_{-}>, <a^{2} \sigma_{+}>, <a^{2} \sigma_{z}>)
      ...
      k1_cavsig3(j, k, a, sz) = -dt * (gamma + 2.0d0 * kappa + i * (wl(j) + wl(k))) * cavsig3(j, k, a, sz) + &
                              ...
                              & dt * gkal(j) * cavsig2(k, a, sm) + &
                              & dt * gkal(k) * cavsig2(j, a, sm)
      ! Third-order: at and \siga (<at^{2}, \sigma_{-}>, <at^{2} \sigma_{+}>, <at^{2} \sigma_{z}>)
      ...
      k1_cavsig3(j, k, at, sz) =  -dt * (gamma + 2.0d0 * kappa - i * (wl(j) + wl(k))) * cavsig3(j, k, at, sz) + &
                               ...
                               & dt * CONJG(gkal(j)) * cavsig2(k, at, sp) + &
                               & dt * CONJG(gkal(k)) * cavsig2(j, at, sp)
      ! Third-order: ata and \sig (<ata, \sigma_{-}>, <ata \sigma_{+}>, <ata \sigma_{z}>)
      ...
      ! Cycle through modes
      DO l = -N, N
        ! Third-order: Cavity (<a^{\dagger} a^{2}>, <a^{\dagger 2} a>)
        ...
        k1_cavsig4(j, k, l, a, sz) = -dt * (gamma + 3.0d0 * kappa - i * (wl(j) - wl(k) - wl(l))) * cavsig4(j, k, l, a, sz) + &
                                   ...
                                   & dt * gkal(k) * cavsig3(j, l, ata, sm) + &
                                   & dt * gkal(l) * cavsig3(j, k, ata, sm)
        ! Fourth-order: at and sigma (<a^{\dagger} a^{\dagger} a \sigma_{-+z}>)
...
IF (N == 0) THEN
  ! Print some operator moments
  ! PRINT*, "~~~~ SECOND-ORDER STEADY-STATE MOMENTS"
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<a a>_ss = ", cav2_ss(0, 0, a)
  ! PRINT*, "<at at>_ss = ", cav2_ss(0, 0, at)
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<a sm>_ss = ", cavsig2_ss(0, a, sm)
  ! PRINT*, "<a sp>_ss = ", cavsig2_ss(0, a, sp)
  ! PRINT*, "<a sz>_ss = ", cavsig2_ss(0, a, sz)
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<at sm>_ss = ", cavsig2_ss(0, at, sm)
  ! PRINT*, "<at sp>_ss = ", cavsig2_ss(0, at, sp)
  ! PRINT*, "<at sz>_ss = ", cavsig2_ss(0, at, sz)
  ! PRINT*, "--------------------------------"
  ! Third order
  ! PRINT*, "~~~~ THIRD-ORDER STEADY-STATE MOMENTS ~~~~"
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<at_{0} a_{0} a_{0}>_ss = ", cav3_ss(0, 0, 0, a)
  ! PRINT*, "<at_{0} at_{0} a_{0}>_ss = ", cav3_ss(0, 0, 0, at)
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<a a sm>_ss = ", cavsig3(0, 0, a, sm)
  ! PRINT*, "<a a sp>_ss = ", cavsig3(0, 0, a, sp)
  ! PRINT*, "<a a sz>_ss = ", cavsig3(0, 0, a, sz)
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<at at sm>_ss = ", cavsig3(0, 0, at, sm)
  ! PRINT*, "<at at sp>_ss = ", cavsig3(0, 0, at, sp)
  ! PRINT*, "<at at sz>_ss = ", cavsig3(0, 0, at, sz)
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<at a sm>_ss = ", cavsig3(0, 0, ata, sm)
  ! PRINT*, "<at a sp>_ss = ", cavsig3(0, 0, ata, sp)
  ! PRINT*, "<at a sz>_ss = ", cavsig3(0, 0, ata, sz)
  ! PRINT*, "--------------------------------"
  ! Fourth order
  PRINT*, "~~~~ FOURTH-ORDER STEADY-STATE MOMENTS ~~~~"
  PRINT*, "--------------------------------"
  PRINT*, "<at at a a>_ss = ", cav4_ss(0, 0, 0, 0)
  PRINT*, "--------------------------------"
  PRINT*, "<at at a sm>_ss = ", cavsig4_ss(0, 0, 0, at, sm)
  PRINT*, "<at at a sp>_ss = ", cavsig4_ss(0, 0, 0, at, sp)
  PRINT*, "<at at a sz>_ss = ", cavsig4_ss(0, 0, 0, at, sz)
  PRINT*, "--------------------------------"
  PRINT*, "<at a a sm>_ss = ", cavsig4_ss(0, 0, 0, a, sm)
  PRINT*, "<at a a sp>_ss = ", cavsig4_ss(0, 0, 0, a, sp)
  PRINT*, "<at a a sz>_ss = ", cavsig4_ss(0, 0, 0, a, sz)
  PRINT*, "--------------------------------"
END IF
...
IF (N == 1) THEN
  ...
  PRINT*, "~~~~ THIRD-ORDER STEADY-STATE MOMENTS ~~~~"
  PRINT*, "--------------------------------"
  PRINT*, "<at_{-1} a_{0} a_{1}>_ss = ", cav3_ss(-1, 0, 1, a)
  PRINT*, "<at_{-1} at_{0} a_{1}>_ss = ", cav3_ss(-1, 0, 1, at)
  PRINT*, "--------------------------------"
  ! PRINT*, "<a a sm>_ss = ", cavsig3(a, sm)
  ! PRINT*, "<a a sp>_ss = ", cavsig3(a, sp)
  ! PRINT*, "<a a sz>_ss = ", cavsig3(a, sz)
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<at at sm>_ss = ", cavsig3(at, sm)
  ! PRINT*, "<at at sp>_ss = ", cavsig3(at, sp)
  ! PRINT*, "<at at sz>_ss = ", cavsig3(at, sz)
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<at a sm>_ss = ", cavsig3(ata, sm)
  ! PRINT*, "<at a sp>_ss = ", cavsig3(ata, sp)
  ! PRINT*, "<at a sz>_ss = ", cavsig3(ata, sz)
  ! PRINT*, "--------------------------------"
  ! Fourth order
  PRINT*, "~~~~ FOURTH-ORDER STEADY-STATE MOMENTS ~~~~"
  PRINT*, "--------------------------------"
  PRINT*, "<at_{-1} at_{0} a_{1} a_{0}>_ss = ", cav4_ss(-1, 0, 1, 0)
  ! PRINT*, "--------------------------------"
  ! PRINT*, "<at at a sm>_ss = ", cavsig4_ss(0, 0, 0, at, sm)
  ! PRINT*, "<at at a sp>_ss = ", cavsig4_ss(0, 0, 0, at, sp)
  ! PRINT*, "<at at a sz>_ss = ", cavsig4_ss(0, 0, 0, at, sz)
...
IF (tau1_max > 0.0) THEN
  ...
  DO t = 0, tau_steps
    ...
    !----------------------!
    ! Initialise Runge-Kutta Vectors
    ! First-order equations with Second-order arrays
    k1_cavsig2 = 0.0d0
    k1_cav2 = 0.0d0
    ...
    DO k = -N, N
      ...
      DO j = -N, N
        ! First order: Bloch
        k1_cavsig2(k, a, sz) = k1_cavsig2(k, a, sz) - dt * gamma * cav2_ss(j, k, ata)
        ! First order: Cavity mode (<a>, <at>)
    ...
    DO k = -N, N
      ...
      DO j = -N, N
        ! First order: Bloch
        k2_cavsig2(k, a, sz) = k2_cavsig2(k, a, sz) - dt * gamma * cav2_ss(j, k, ata)
        ! First order: Cavity mode (<a>, <at>)
    ...
    DO k = -N, N
      ...
      DO j = -N, N
        ! First order: Bloch
        k3_cavsig2(k, a, sz) = k3_cavsig2(k, a, sz) - dt * gamma * cav2_ss(j, k, ata)
        ! First order: Cavity mode (<a>, <at>)
    ...
    DO k = -N, N
      ...
      DO j = -N, N
        ! First order: Bloch
        k4_cavsig2(k, a, sz) = k4_cavsig2(k, a, sz) - dt * gamma * cav2_ss(j, k, ata)
        ! First order: Cavity mode (<a>, <at>)
...
IF (tau2_max > 0.0) THEN
  ...
  ! ! First-order moments
  ! bloch = 0.0d0
  ! bloch(sm) = cavsig3_ss(ata, sm); bloch(sp) = cavsig3_ss(ata, sp); bloch(sz) = cavsig3_ss(ata, sz)
  ! cav1 = 0.0d0
  ! cav1(a) = cav3_ss(a)
  ! cav1(at) = cav3_ss(at)
  ...
  ! cav2(ata) = cav4_ss
  ! cavsig2 = 0.0d0
  ! cavsig2(a, sm) = cavsig4_ss(a, sm); cavsig2(a, sp) = cavsig4_ss(a, sp); cavsig2(a, sz) = cavsig4_ss(a, sz)
  ! cavsig2(at, sm) = cavsig4_ss(at, sm); cavsig2(at, sp) = cavsig4_ss(at, sp); cavsig2(at, sz) = cavsig4_ss(at, sz)

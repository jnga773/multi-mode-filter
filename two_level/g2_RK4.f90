...
DO t = 0, tau_steps
  ...
  k1_sigma = dt * (MATMUL(Mat_OG, sigma) + B)
  k2_sigma = dt * (MATMUL(Mat_OG, (sigma + 0.5d0 * k1_sigma)) + B)
  k3_sigma = dt * (MATMUL(Mat_OG, (sigma + 0.5d0 * k2_sigma)) + B)
  k4_sigma = dt * (MATMUL(Mat_OG, (sigma + k3_sigma)) + B)
  ...
  DO j = -N, N
    ...
    B = 0.0d0
    B(1) = 0.0d0
    B(2) = -0.5d0 * gkl(j) * (sigma(sz) + photon_ss)
    B(3) = -gamma * cav1(j, a) + &
         & gkl(j) * sigma(sm)
    ! Calculate k1
    ...
    B = 0.0d0
    B(1) = 0.0d0
    B(2) = -0.5d0 * gkl(j) * ((sigma(sz) + 0.5d0 * k1_sigma(sz)) + photon_ss)
    ...
    B = 0.0d0
    B(1) = 0.0d0
    B(2) = -0.5d0 * gkl(j) * ((sigma(sz) + 0.5d0 * k2_sigma(sz)) + photon_ss)
    ...
    B = 0.0d0
    B(1) = 0.0d0
    B(2) = -0.5d0 * gkl(j) * ((sigma(sz) + k3_sigma(sz)) + photon_ss)
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -0.5d0 * CONJG(gkl(j)) * (sigma(sz) + photon_ss)
    B(2) = 0.0d0
    B(3) = -gamma * cav1(j, at) + &
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -0.5d0 * CONJG(gkl(j)) * ((sigma(sz) + 0.5d0 * k1_sigma(sz)) + photon_ss)
    B(2) = 0.0d0
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -0.5d0 * CONJG(gkl(j)) * ((sigma(sz) + 0.5d0 * k2_sigma(sz)) + photon_ss)
    B(2) = 0.0d0
    ...
    ! Set the non-homogeneous vector
    B = 0.0d0
    B(1) = -0.5d0 * CONJG(gkl(j)) * ((sigma(sz) + k3_sigma(sz)) + photon_ss)
    B(2) = 0.0d0
